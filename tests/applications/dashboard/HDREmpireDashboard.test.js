...
describe("HDR Empire Dashboard - Main Test Suite", () => {
  ...
  describe("Application Launching", () => {
    ...
    test("should handle launch failures", async () => {
      dashboard.launchApplication.mockRejectedValue(
        new Error("Application not found")
      );

      await expect(
        dashboard.launchApplication({ name: "invalid-app" })
  ...
  describe("Error Handling", () => {
    test("should handle system errors gracefully", async () => {
      dashboard.getSystemStatus.mockRejectedValue(
        new Error("System unavailable")
      );

      await expect(dashboard.getSystemStatus()).rejects.toThrow(
        "System unavailable"
      );
    });
