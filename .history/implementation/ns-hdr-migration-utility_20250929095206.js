...
class NSHDRMigrationUtility {
  ...
  async _backupExistingFiles() {
    ...
    for (const file of filesToBackup) {
      ...
      try {
        const exists = await fs.access(sourcePath)
          .then(() => true)
          .catch(() => false);
  ...
  async _updateFileReferences() {
    ...
    for (const file of files) {
      ...
      try {
        ...
        console.warn(`Warning: Could not update references in ${file}: ${error.message}`);
      }
    }
  }
  ...
  async _validateFileStructure() {
    ...
    for (const file of requiredFiles) {
      const exists = await fs.access(path.join(this.sourceDir, file))
        .then(() => true)
        .catch(() => false);
