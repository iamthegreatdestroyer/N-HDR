...
class QdrantKnowledgeStore {
  ...
  async initialize() {
    try {
      ...
      console.error("[QdrantKnowledgeStore] Initialization failed:", err.message);
  ...
  async _buildEncoder() {
    ...
    let x = tf.layers.dense({ units: 256, activation: "relu", name: "enc_dense1" }).apply(input);
    x = tf.layers.dense({ units: this.embeddingDim, activation: "relu", name: "enc_dense2" }).apply(x);

    // L2-normalize so cosine similarity works correctly
    const normLayer = tf.layers.activation({ activation: "linear", name: "enc_norm" });
    x = normLayer.apply(x);
  ...
  async _ensureCollection(name, dim) {
    ...
    try {
      ...
      console.error(`[QdrantKnowledgeStore] Failed to ensure collection "${name}":`, err.message);
      throw err;
    }
  }
   ...
   */
  async searchSimilarCrystals(queryCrystal, topK = DEFAULT_TOP_K, filter = null, collection = CRYSTAL_COLLECTION) {
    ...
    this._stats.avgSearchLatency =
      (this._stats.avgSearchLatency * (this._stats.totalSearches - 1) + latency) /
      this._stats.totalSearches;
   ...
   */
  async searchByFeatures(featureVector, topK = DEFAULT_TOP_K, filter = null, collection = CRYSTAL_COLLECTION) {
    const startTime = Date.now();
   ...
   */
  async searchByDimension(dimension, topK = DEFAULT_TOP_K, collection = CRYSTAL_COLLECTION) {
  ...
  async retrieveAugmentedContext(queryCrystal, opts = {}) {
    ...
    const results = await this.searchSimilarCrystals(queryCrystal, topK * 2, null, collection);
    ...
    if (includeStability) {
      entries = entries.map((e) => ({
        ...e,
        hybridScore: e.score * 0.6 + (e.payload?.stability ?? 0) * 0.25 + (e.payload?.coherence ?? 0) * 0.15,
      }));
    ...
    const totalScore = entries.reduce((sum, e) => sum + (e.hybridScore ?? e.score), 0);
  ...
  _consciousnessTocrystal(state) {
    ...
    const avgDimValue = dimValues.length > 0
      ? dimValues.reduce((s, v) => s + (typeof v === "number" ? v : 0), 0) / dimValues.length
      : 0.5;
...
export { QdrantKnowledgeStore, createQdrantKnowledgeStore, CRYSTAL_COLLECTION, EXPERTISE_COLLECTION };
