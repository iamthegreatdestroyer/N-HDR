...
class KnowledgeCrystallizer {
  ...
  async initialize() {
    ...
    try {
      ...
      console.warn("[KnowledgeCrystallizer] Vector store unavailable, continuing without RAG:", err.message);
      this.vectorStoreReady = false;
    }
  ...
  async _validateAndStore(crystals) {
    ...
    if (this.vectorStoreReady && validated.length > 0) {
      try {
        ...
        console.warn("[KnowledgeCrystallizer] Vector embedding failed (non-fatal):", err.message);
      }
    }
  }
  ...
  async retrieveContext(consciousnessState, opts = {}) {
    if (!this.vectorStoreReady) {
      return { contextEntries: [], totalScore: 0, metadata: { error: "vector store offline" } };
    }
    ...
    if (!this.vectorStoreReady) {
      ...
      for (const [id, crystal] of this.crystalPatterns) {
        if (crystal.pattern?.dimension === dimension) {
          results.push({ crystalId: id, score: 1.0, payload: { dimension, stability: crystal.stability?.overall ?? 0 } });
        }
