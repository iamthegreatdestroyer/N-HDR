...
class StatePreservation {
  ...
  _startIntegrityChecks() {
    setInterval(async () => {
      const now = Date.now();
      if (now - this.lastIntegrityCheck >= this.options.integrityCheckInterval) {
  ...
  async _pruneSnapshots() {
    while (this.snapshotTimestamps.length > this.options.maxSnapshots) {
      ...
      const oldestSnapshot = Array.from(this.snapshots.values())
        .find(s => s.timestamp === oldestTimestamp);
      ...
      if (oldestSnapshot) {
        ...
        try {
          ...
          console.warn(`Failed to delete key for snapshot ${oldestSnapshot.id}`);
        }
      }
    }
  }
