...
class ProbabilityPathVisualizer extends EventEmitter {
  ...
  async initialize() {
    try {
      ...
      throw new Error(`Probability visualizer initialization failed: ${error.message}`);
    }
  }
  ...
  async explorePossibilities(crystals, options = {}) {
    ...
    try {
      ...
      for (const crystal of crystals) {
        const crystalPathways = await this._exploreFromCrystal(
          crystal,
          depth,
          { swarm }
        );
  ...
  async _exploreFromCrystal(crystal, depth, options = {}) {
    ...
    for (let i = 0; i < this.config.maxStates; i++) {
      const pathway = await this._explorePathway(
        initialState,
        depth,
        { swarm, pathIndex: i }
      );
  ...
  _calculateConfidence(states) {
    const avgProbability = states.reduce((sum, s) => sum + (s.probability || 1), 0) / states.length;
  ...
  _generateVisualizationMetadata(pathways, nodes, edges) {
    return {
      ...
      avgProbability: pathways.reduce((sum, p) => sum + p.probability, 0) / pathways.length,
  ...
  _generateExplorationId() {
    return `exploration-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
  }
