...
class ConsciousnessWorkbench extends EventEmitter {
  ...
  async initialize() {
    ...
    try {
      ...
      throw new Error(`Consciousness Workbench initialization failed: ${error.message}`);
    }
  }
  ...
  async transformState(stateId, transformation) {
    ...
    try {
      ...
      const transformed = await this.transformationTool.transform(state, transformation, {
        swarm: await this.swarmEngine.getActiveSwarm()
      });
