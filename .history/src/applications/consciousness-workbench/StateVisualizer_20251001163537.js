...
class StateVisualizer extends EventEmitter {
  ...
  async visualize(state, options = {}) {
    ...
    try {
      ...
      const edges = showConnections
        ? await this._generateEdges(state, nodes)
        : [];
      ...
      const visualization = {
        ...
        metadata,
        colorScheme,
        animation: this.config.animationEnabled
          ? this._generateAnimation(nodes)
          : null,
      };
  ...
  async _generateEdges(state, nodes) {
    ...
    for (let i = 0; i < layerNodes.length - 1; i++) {
      edges.push({
        ...
        strength:
          (layerNodes[i].entanglement + layerNodes[i + 1].entanglement) / 2,
      });
    }
  ...
  _getNodeColor(node, colorScheme) {
    if (colorScheme === "consciousness") {
      switch (node.type) {
        ...
        case "memory":
          return node.subtype === "short-term"
            ? "#ff9800"
            : node.subtype === "long-term"
            ? "#ff5722"
            : "#ffc107";
        case "pattern":
          return node.subtype === "behavioral"
            ? "#4caf50"
            : node.subtype === "cognitive"
            ? "#2196f3"
            : "#e91e63";
        default:
          return "#9e9e9e";
