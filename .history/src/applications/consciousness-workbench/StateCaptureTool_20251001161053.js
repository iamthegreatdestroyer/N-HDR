...
class StateCaptureTool extends EventEmitter {
  ...
  async capture(source, options = {}) {
    ...
    try {
      ...
      const quantumLayers = await this._captureQuantumLayers(source, layers, swarm);

      // Capture memory if enabled
      const memory = includeMemory ? await this._captureMemory(source, swarm) : null;

      // Capture patterns if enabled
      const patterns = includePatterns ? await this._capturePatterns(source, swarm) : null;
  ...
  _calculateAvgCaptureTime() {
    ...
    const total = this.captureHistory.reduce((sum, c) => sum + c.captureTime, 0);
