...
class PerformanceMonitor extends EventEmitter {
    ...
    getPerformanceSummary() {
        ...
        for (const [component, metrics] of this.metrics) {
            summary.components[component] = {
                ...
                alerts: this.getActiveAlerts().filter(alert => alert.component === component)
            };
        }
    ...
    _updateMetrics(newMetrics) {
        for (const [component, data] of Object.entries(newMetrics)) {
            if (!this.metrics.has(component)) {
                ...
                // Trim history
                while (metrics.history.length > 0 && 
                       Date.now() - metrics.history[0].timestamp > this.options.metricRetention * 1000) {
    ...
    _generateAlertMessage(component, status) {
        ...
        switch (component) {
            ...
            case 'memory':
                const memUsage = (metrics.current.used / metrics.current.total) * 100;
                return `${status.toUpperCase()}: Memory usage at ${memUsage.toFixed(1)}%`;

            case 'thermal':
                return `${status.toUpperCase()}: Temperature at ${metrics.current.temperature}°C`;

            case 'quantum':
                return `${status.toUpperCase()}: Entropy quality at ${metrics.current.entropyQuality.toFixed(2)}`;
