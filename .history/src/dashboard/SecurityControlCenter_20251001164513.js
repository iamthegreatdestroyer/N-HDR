...
export class SecurityControlCenter extends EventEmitter {
  ...
  async createZone(name, config = {}) {
    try {
      ...
      const zoneResult = await this.commander.executeCommand(
        "void-blade-hdr",
        "createSecurityZone",
        {
          name,
  ...
  async protect(resourceId, options = {}) {
    try {
      ...
      const protectionResult = await this.commander.executeCommand(
        "void-blade-hdr",
        "protect",
        {
          resourceId,
  ...
  async _respondToThreat(threat) {
    try {
      ...
      this._logAudit("threat", "mitigated", {
        id: threat.id,
        type: threat.type,
      });
    } catch (error) {
  ...
  async shutdown() {
    try {
      ...
      const unprotectPromises = Array.from(this.protectedResources.keys()).map(
        (resourceId) =>
