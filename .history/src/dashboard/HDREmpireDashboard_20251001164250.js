  ...
  async initialize(commander, options = {}) {
    try {
      ...
      this.emit('initialized', {
        timestamp: Date.now(),
        subsystems: Object.keys(this).filter(key => 
          this[key] && typeof this[key] === 'object' && this[key].isInitialized
        )
      });
  ...
  _getOverviewData() {
    return {
      systems: {
        ...
        active: Object.values(this.state.systems).filter(s => s.status === 'active').length,
        status: this._getSystemsOverallStatus()
      },
      applications: {
        total: Object.keys(this.state.applications).length,
        running: Object.values(this.state.applications).filter(a => a.status === 'running').length
      },
      security: {
        level: this.securityCenter?.getSecurityLevel() || 'unknown',
        alerts: Object.values(this.state.security).filter(e => e.severity === 'high' || e.severity === 'critical').length
      },
      swarms: {
        total: Object.keys(this.state.swarms).length,
        active: Object.values(this.state.swarms).filter(s => s.status === 'active').length,
        bots: Object.values(this.state.swarms).reduce((sum, s) => sum + (s.botCount || 0), 0)
      },
      resources: {
