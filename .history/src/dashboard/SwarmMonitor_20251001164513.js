...
export class SwarmMonitor extends EventEmitter {
  ...
  async _updateSwarmInfo(swarmData) {
    ...
    swarm.taskCompletionRate =
      swarm.taskCount > 0 ? swarm.completedTasks / swarm.taskCount : 0;
  ...
  _updateMetrics() {
    ...
    this.metrics.averageEfficiency =
      this.swarms.size > 0
        ? Array.from(this.swarms.values()).reduce(
  ...
  _getSwarmConnections(swarm) {
    ...
    for (const other of this.swarms.values()) {
      if (
        other.id !== swarm.id &&
        this._areTargetsRelated(swarm.target, other.target)
      ) {
  ...
  getAnalytics() {
    ...
    return {
      ...
      averageEfficiency:
        swarms.reduce((sum, s) => sum + (s.efficiency || 0), 0) / swarms.length,
      ...
      averageReplicationRate:
        swarms.reduce((sum, s) => sum + (s.replicationRate || 0), 0) /
        swarms.length,
