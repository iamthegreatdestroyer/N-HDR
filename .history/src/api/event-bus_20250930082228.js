...
class EventBus {
  ...
  async publish(event, data) {
    try {
      ...
      const shouldBuffer = Array.from(subscribers)
        .some(sub => sub.options.buffer);
  ...
  async publishAll(events) {
    const publications = Object.entries(events)
      .map(([event, data]) => this.publish(event, data));
  ...
  subscribePattern(pattern, handler, options = {}) {
    const matchingEvents = Array.from(this._subscribers.keys())
