...
class HDRMonitoringDashboard extends EventEmitter {
  ...
  updateTaskProgress(phase, taskUpdate) {
    ...
    if (taskUpdate.status === "completed") {
      ...
      phaseMetrics.tasks.inProgress = Math.max(
        0,
        phaseMetrics.tasks.inProgress - 1
      );
    ...
    phaseMetrics.progress =
      phaseMetrics.tasks.total > 0
  ...
  updateResourceMetrics(resources) {
    ...
    if (resources.network !== undefined) {
      this.resourceMetrics.network.push({
        timestamp,
  ...
  updateMetrics() {
    ...
    this.updateResourceMetrics({
      cpu: Math.min(
        100,
        (baseLoad + swarmLoad + taskLoad) * 100 + Math.random() * 10
      ),
      memory: Math.min(
        100,
        40 + this.swarmRegistry.size * 5 + Math.random() * 10
      ),
      network: Math.min(
        100,
        20 + this.getTotalActiveTasks() * 0.5 + Math.random() * 15
      ),
  ...
  displayDashboard() {
    ...
    console.log(
      "║" +
        this.centerText("HDR EMPIRE PROTOCOL - EXECUTION DASHBOARD", 78) +
        "║"
    );
    ...
    for (const [phaseKey, phase] of Object.entries(this.metrics)) {
      ...
      console.log(
        `│ ${statusIcon} ${phase.name.padEnd(
          42
        )} ${progressBar} ${phase.progress.toFixed(0).padStart(3)}% │`
      );
      console.log(
        `│   Tasks: ${phase.tasks.completed}/${phase.tasks.total} completed, ${phase.tasks.inProgress} active, ${phase.tasks.failed} failed`.padEnd(
          78
        ) + "│"
      );
    ...
    for (const [name, value] of Object.entries(resources)) {
      ...
      console.log(
        `│ ${name.padEnd(10)} ${bar} ${value
          .toFixed(1)
          .padStart(5)}% ${color}`.padEnd(87) + "│"
      );
    }
    ...
    console.log(
      `│ Active: ${activeTasks} | Completed: ${completedTasks} | Completion Rate: ${completionRate}%`.padEnd(
        79
      ) + "│"
    );
    console.log("└" + "─".repeat(78) + "┘");
   ...
   */
  getTotalBots() {
    return Array.from(this.swarmRegistry.values()).reduce(
      (sum, swarm) => sum + (swarm.botCount || 0),
      0
    );
  }
   ...
   */
  getTotalTasks() {
    return Object.values(this.metrics).reduce(
      (sum, phase) => sum + phase.tasks.total,
      0
    );
  }
  ...
  getTotalActiveTasks() {
    return Object.values(this.metrics).reduce(
      (sum, phase) => sum + phase.tasks.inProgress,
      0
    );
  }
  ...
  getTotalCompletedTasks() {
    return Object.values(this.metrics).reduce(
      (sum, phase) => sum + phase.tasks.completed,
      0
    );
  }
  ...
  getSnapshot() {
    return {
      ...
      tasks: {
        ...
        failed: Object.values(this.metrics).reduce(
          (sum, phase) => sum + phase.tasks.failed,
          0
        ),
      },
    };
...
- **Tasks Completed:** ${snapshot.tasks.completed} (${(
...
- **Tasks:** ${this.metrics.phase1.tasks.completed}/${
      this.metrics.phase1.tasks.total
    }
...
- **Tasks:** ${this.metrics.phase2.tasks.completed}/${
      this.metrics.phase2.tasks.total
    }
...
- **Tasks:** ${this.metrics.phase3.tasks.completed}/${
      this.metrics.phase3.tasks.total
    }
