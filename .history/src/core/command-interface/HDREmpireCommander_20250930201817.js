...
class HDREmpireCommander {
    ...
    async initialize(parameters) {
        try {
            ...
            throw new Error(`Command interface initialization failed: ${error.message}`);
        }
    }
    ...
    async _getSystemSecurity(registration) {
        ...
        const systemZone = zones.find(zone => 
            zone.id.includes(registration.id)
        );
    ...
    getCommandHistory(systemId = null) {
        ...
        if (systemId) {
            history = history.filter(entry => 
                entry.command.systemId === systemId
            );
        }
