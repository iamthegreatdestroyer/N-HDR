...
class SwarmOrchestrator extends EventEmitter {
  ...
  async start() {
    try {
      ...
      console.log(`${this.name} started with ${this.agents.size} initial agents`);
  ...
  async spawnAgent(config = {}) {
    ...
    try {
      ...
      console.log(`Spawned agent: ${agent.agent_id} (total: ${this.agents.size}/${this.config.max_agents})`);
  ...
  async runHealthCheck() {
    ...
    for (const [, agent] of this.agents) {
      ...
      statuses.push({
        ...
        state: status.state,
        healthy:
          status.state === "ready" || status.state === "executing",
      });
    }
    ...
    this.swarm_health =
      health_percentage > 80 ? "healthy" : health_percentage > 50 ? "degraded" : "unhealthy";
      ...
      tasks: {
        ...
        avg_execution_ms:
          this.metrics.tasks_completed > 0
            ? this.metrics.total_execution_time_ms / this.metrics.tasks_completed
            : 0,
      },
      metrics: this.metrics,
    };
