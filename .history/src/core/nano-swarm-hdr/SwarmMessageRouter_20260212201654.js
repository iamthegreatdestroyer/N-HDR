...
class SwarmMessageRouter extends EventEmitter {
  ...
  async deliverToAgent(envelope, routing_context) {
    const target_agent = this.orchestrator.agents.get(envelope.to_agent_id);
    ...
    // Retry loop
    for (let attempt = 0; attempt < this.config.max_retry_attempts; attempt++) {
      try {
  ...
  recordMessage(envelope, routing_context) {
    ...
    if (this.message_history.length > this.config.message_history_size) {
