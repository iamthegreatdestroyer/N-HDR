...
class SwarmMetricsCollector extends EventEmitter {
  ...
  collectMetrics() {
    ...
    try {
      ...
      metrics.uptime_seconds = (new Date() - this.created_at) / 1000;

      // Task metrics from orchestrator
      const swarm_status = this.orchestrator.getSwarmStatus();
      if (swarm_status) {
        metrics.completed_tasks = swarm_status.metrics?.tasks_completed || 0;
      ...
      for (const [agent_id, agent] of this.orchestrator.agents) {
        ...
        if (agent_status.last_task_duration_ms) {
        ...
        total_utilization +=
          (agent_status.active_tasks / Math.max(agent_status.task_count, 1)) *
          100;
        ...
        agent_metric.error_count =
          agent_status.task_count - agent_status.success_count;
      ...
      if (metrics.swarm_size > 0) {
        ...
        metrics.memory_utilization_percent = total_memory / metrics.swarm_size;
      ...
      if (agent_latencies.length > 0) {
        const sorted_latencies = agent_latencies.sort((a, b) => a - b);
        metrics.average_task_duration_ms =
          sorted_latencies.reduce((a, b) => a + b, 0) / sorted_latencies.length;

        for (const percentile of this.config.percentile_buckets) {
          const index = Math.ceil((percentile / 100) * sorted_latencies.length);
      ...
      if (this.collection_count > 1) {
        const prev_metrics =
          this.historical_metrics[this.historical_metrics.length - 1];
        if (prev_metrics) {
          const time_delta_seconds =
            (metrics.timestamp - prev_metrics.timestamp) / 1000;
          const task_delta =
            metrics.completed_tasks - prev_metrics.completed_tasks;
      ...
      if (this.historical_metrics.length > this.config.history_size) {
  ...
  calculateHealthScore(metrics) {
    ...
    if (metrics.task_latencies.p99 > 1000) {
      score -= Math.min(10, (metrics.task_latencies.p99 - 1000) / 100);
    }
  ...
  detectAnomalies(current_metrics) {
    ...
    if (
      ...
      means.task_success_rate -
        this.config.anomaly_std_devs * std_devs.task_success_rate
    ) {
      anomalies.push({
  ...
  getStatistics(seconds = 300) {
    ...
    return {
      ...
      average_cpu_utilization:
        window_metrics.reduce((sum, m) => sum + m.cpu_utilization_percent, 0) /
        window_metrics.length,
