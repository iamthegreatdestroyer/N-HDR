...
class SwarmAgent extends EventEmitter {
  constructor(config = {}) {
    ...
    this.agent_id =
      config.agent_id ||
      `agent-${Date.now()}-${crypto.randomBytes(4).toString("hex")}`;
    ...
    this.on("message_received", (msg) => {
      console.log(
        `[${this.agent_id}] Message from ${msg.from_agent_id}: ${msg.content_preview}`,
      );
    });
    ...
    this.on("task_error", (task, error) => {
      console.error(
        `[${this.agent_id}] Task ${task.id} failed: ${error.message}`,
      );
    });
  }
  ...
  async receiveMessage(envelope) {
    ...
    switch (envelope.content_type) {
      ...
      default:
        ...
        console.log(
          `[${this.agent_id}] Unknown message type: ${envelope.content_type}`,
        );
    }
  }
  ...
  async persistState() {
    try {
      ...
      await fs.writeFile(
        state_file,
        JSON.stringify(serializable_state, null, 2),
      );
      return true;
    } catch (e) {
  ...
  async loadPersistedState() {
    try {
      ...
      console.log(
        `[${this.agent_id}] Loaded persisted state: ${persisted.task_count} tasks completed`,
      );
      return true;
    } catch (e) {
  ...
  async terminate() {
    ...
    console.log(
      `[${this.agent_id}] Terminated (${this.success_count}/${this.task_count} tasks successful)`,
    );
  }
  ...
  getStatus() {
    return {
      ...
      success_count: this.success_count,
      success_rate:
        this.task_count > 0 ? (this.success_count / this.task_count) * 100 : 0,
      active_tasks: this.active_tasks.size,
