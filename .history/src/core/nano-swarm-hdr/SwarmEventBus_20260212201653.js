...
class SwarmEventBus extends EventEmitter {
  ...
  async publishEvent(topic, data = {}, options = {}) {
    ...
    if (this.config.enable_history) {
      ...
      // Trim history
      if (this.event_history.length > this.config.event_history_size) {
    ...
    for (const subscription of all_subs) {
      try {
        ...
        if (subscription.once_only) {
          ...
          if (index > -1) {
            const topic_subs = this.subscriptions.get(subscription.topic);
  ...
  moveToDeadLetterQueue(event, subscription, error) {
    ...
    if (this.dead_letter_queue.length > this.config.dead_letter_queue_size) {
  ...
  getEventHistory(topic = null, limit = 100) {
    ...
    return this.event_history.filter((e) => e.topic === topic).slice(-limit);
  }
  ...
  queryEventHistory(filters = {}) {
    ...
    if (filters.since) {
      results = results.filter((e) => e.timestamp >= new Date(filters.since));
    }

    if (filters.until) {
      results = results.filter((e) => e.timestamp <= new Date(filters.until));
    }
   ...
   */
  getEventStatistics() {
    const uptime_seconds = (new Date() - this.created_at) / 1000;

    return {
      uptime_seconds,
      total_events_published: Object.values(this.event_counts).reduce(
        (a, b) => a + b,
        0,
      ),
      event_count_by_topic: { ...this.event_counts },
      total_subscriptions: Array.from(this.subscriptions.values()).reduce(
        (sum, subs) => sum + subs.length,
        0,
      ),
      subscriptions_by_topic: Array.from(this.subscriptions.entries()).reduce(
        (obj, [topic, subs]) => {
  ...
  async replayEvents(topic, callback, limit = 50) {
    ...
    for (const event of events) {
      try {
        ...
        console.error(`Error replaying event ${event.event_id}:`, e);
      }
    }
