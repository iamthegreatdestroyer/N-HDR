...
class SwarmTaskQueue extends EventEmitter {
   ...
   * Mark task as failed
   */
  markTaskFailed(task_id, error, options = {}) {
    const task = this.in_progress.get(task_id);
    if (!task) {
      return false;
    }
  ...
  handleTaskTimeout(task_id) {
    ...
    this.markTaskFailed(task_id, new Error("Task timeout"), {
      timed_out: true,
    });
    ...
    return {
      ...
      average_task_duration_ms:
        this.history.length > 0
          ? this.history.reduce((sum, t) => sum + (t.duration_ms || 0), 0) /
            this.history.length
          : 0,
    };
  }
    ...
    return {
      ...
      task_success_rate:
        this.task_counts.total > 0
          ? (this.task_counts.completed / this.task_counts.total) * 100
          : 0,
      task_latencies: {
  ...
  queryTaskHistory(filters = {}) {
    ...
    if (filters.since) {
      results = results.filter(
        (t) => t.completed_at >= new Date(filters.since),
      );
    }

    if (filters.correlation_id) {
      results = results.filter(
        (t) => t.correlation_id === filters.correlation_id,
      );
    }
