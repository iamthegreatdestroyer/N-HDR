...
class IntuitionEngine {
  ...
  async process(patterns, threshold = this.confidenceThreshold) {
    try {
      ...
      const insights = await this._generateInsights(connections, threshold);
   ...
   */
  async _exploreConnections(pattern, allPatterns, connections, visited, depth) {
    if (
      depth >= this.correlationDepth ||
    ...
    for (const other of allPatterns) {
      if (pattern === other || visited.has(other.recognition.signature.hash)) {
        continue;
      }
  ...
  async _calculateSimilarity(pattern1, pattern2) {
    ...
    return (
      nodeSimilarity * 0.4 + connectionSimilarity * 0.4 + typeSimilarity * 0.2
    );
  }
  ...
  _clusterConnections(connections) {
    ...
    for (const connection of connections) {
      ...
      const cluster = this._expandCluster(connection, connections, visited);
      if (cluster.length > 0) {
  ...
  _expandCluster(seed, connections, visited) {
    ...
    while (queue.length > 0) {
      ...
      const related = this._findRelatedConnections(current, connections);
  ...
  async _generateClusterInsight(cluster) {
    const patterns = new Set(cluster.flatMap((conn) => conn.patterns));
  ...
  _calculateClusterConfidence(cluster) {
    ...
    const weightedConfidence =
      confidences.reduce((sum, conf, i) => sum + conf * strengths[i], 0) /
  ...
  async _enhanceInsight(insight, factor) {
    return {
      ...
      confidence: Math.min(1, insight.confidence * Math.sqrt(factor)),
      enhancement: {
        factor,
        timestamp: Date.now(),
      },
  ...
  _generateInsightKey(insight) {
    const patterns = insight.patterns
      .map((p) => p.recognition.signature.hash)
      .sort();
