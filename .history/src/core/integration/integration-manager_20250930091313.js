...
class IntegrationManager {
    ...
    getStatus() {
        return {
            ...this._state,
            components: Array.from(this._components.entries()).map(([name, component]) => ({
                name,
    ...
    async _startComponents() {
        ...
        const metrics = this._components.get('metricsCollector').instance;
        metrics.recordMetric('system.startup.duration', 
        ...
        try {
            while (pending.size > 0) {
                ...
                for (const name of pending) {
                    ...
                    const hasDependents = Array.from(this._dependencies.entries())
                        .some(([depName, deps]) => 
                            deps.has(name) && !stopped.has(depName));
