...
class ApiServer {
    ...
    _setupMiddleware() {
        ...
        this.app.use((req, res, next) => {
            ...
            res.on('finish', () => {
                ...
                const totalRequests = this._requestMetrics.success + this._requestMetrics.failed;
                this._requestMetrics.avgResponseTime = 
                    ((this._requestMetrics.avgResponseTime * (totalRequests - 1)) + duration) / totalRequests;
    ...
    _setupRoutes() {
        ...
        // System routes
        this.app.get('/api/system/status', this._authenticateToken.bind(this), this._handleSystemStatus.bind(this));
        this.app.get('/api/system/metrics', this._authenticateToken.bind(this), this._handleSystemMetrics.bind(this));

        // Consciousness routes
        this.app.get('/api/consciousness/layers', this._authenticateToken.bind(this), this._handleGetLayers.bind(this));
        this.app.post('/api/consciousness/preserve', this._authenticateToken.bind(this), this._handlePreserve.bind(this));
        this.app.post('/api/consciousness/transfer', this._authenticateToken.bind(this), this._handleTransfer.bind(this));

        // Swarm routes
        this.app.get('/api/swarm/nodes', this._authenticateToken.bind(this), this._handleGetNodes.bind(this));
        this.app.post('/api/swarm/optimize', this._authenticateToken.bind(this), this._handleOptimize.bind(this));

        // Quantum routes
        this.app.get('/api/quantum/state', this._authenticateToken.bind(this), this._handleQuantumState.bind(this));
        this.app.post('/api/quantum/operate', this._authenticateToken.bind(this), this._handleQuantumOperation.bind(this));
    }
    ...
    async _handleSystemStatus(req, res) {
        try {
            ...
            const status = await new Promise((resolve) => {
                const subId = eventBus.subscribe('system.status.response', 
                    (data) => {
    ...
    async _handleSystemMetrics(req, res) {
        try {
            ...
            const metrics = await new Promise((resolve) => {
                const subId = eventBus.subscribe('system.metrics.response', 
                    (data) => {
                        eventBus.unsubscribe(subId);
                        resolve(data);
                    }, 
