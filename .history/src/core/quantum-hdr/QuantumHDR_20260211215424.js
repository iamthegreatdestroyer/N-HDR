...
class QuantumHDR {
  constructor(config = {}) {
    ...
    this.monteCarloEngine = new MonteCarloEngine({
      ...
      criterion:
        config.monteCarlo?.criterion ??
        ConvergenceCriterion.CONFIDENCE_INTERVAL,
      ...
      const mcEstimate = await this.monteCarloEngine.simulate(
        ...
        (sample) => {
          ...
          for (const outcome of optimizedOutcomes) {
            const sampleKey = Object.keys(sample).find(
              (k) => !k.startsWith("_"),
            );
            const overlap =
              1 - Math.abs((sample[sampleKey] ?? 0.5) - outcome.probability);
  ...
  async assessPathwayRisk(samplesPerPathway = 10_000) {
    ...
    const pathways =
      this.currentState.pathways instanceof Map
