...
class DecisionTreeExplorer {
    ...
    async navigatePathway(pathwayId, parameters) {
        ...
        try {
            const navigation = await this._navigatePath(
                pathway.paths,
                parameters
            );
    ...
    async analyzeOutcomes(pathwayId) {
        ...
        try {
            ...
            const recommendations = await this._generateOutcomeRecommendations(outcomes);
    ...
    _calculateTreeDepth(node) {
        ...
        return 1 + Math.max(...node.children.map(child => 
            this._calculateTreeDepth(child)
        ));
    }
    ...
    _findBestPath(paths, parameters) {
        ...
        }, { path: paths[0], score: -Infinity }).path;
    }
