 ...
 * Phase 9.5 — Monte Carlo simulation engine integration.
 */
...
import { MonteCarloEngine, SamplingStrategy, ConvergenceCriterion } from "./monte-carlo-engine.js";
...
class QuantumHDR {
  constructor(config = {}) {
    ...
    this.monteCarloEngine = new MonteCarloEngine({
      maxSamples: config.monteCarlo?.maxSamples ?? 50_000,
      batchSize: config.monteCarlo?.batchSize ?? 2_000,
      convergenceThreshold: config.monteCarlo?.convergenceThreshold ?? 0.001,
      confidenceLevel: config.monteCarlo?.confidenceLevel ?? 0.95,
      strategy: config.monteCarlo?.strategy ?? SamplingStrategy.IMPORTANCE,
      criterion: config.monteCarlo?.criterion ?? ConvergenceCriterion.CONFIDENCE_INTERVAL,
      seed: config.monteCarlo?.seed ?? null,
    });
  ...
  async exploreFutures(parameters) {
    ...
    try {
      ...
      // Monte Carlo enhanced probability estimation
      const mcEstimate = await this.monteCarloEngine.simulate(
        this.currentState.probability,
        (sample) => {
          let score = 0;
          for (const outcome of optimizedOutcomes) {
            const sampleKey = Object.keys(sample).find(k => !k.startsWith('_'));
            const overlap = 1 - Math.abs((sample[sampleKey] ?? 0.5) - outcome.probability);
            score += overlap * outcome.confidence;
          }
          return score / Math.max(optimizedOutcomes.length, 1);
        },
        { maxSamples: 10_000, strategy: SamplingStrategy.IMPORTANCE }
      );
      ...
      return {
        ...
        monteCarlo: {
          estimatedMean: mcEstimate.mean,
          confidenceInterval: mcEstimate.confidenceInterval,
          sampleCount: mcEstimate.sampleCount,
          converged: mcEstimate.converged,
        },
  ...
  /**
   * Run a Monte Carlo simulation over the current quantum probability space.
   * @param {Function} [payoff]  – Custom payoff function (sample → number)
   * @param {Object}   [opts]    – Override engine defaults per-run
   * @returns {Promise<Object>} Full simulation results
   */
  async runMonteCarloSimulation(payoff, opts = {}) {
    if (!this.currentState) {
      throw new Error("Quantum space not initialized");
    }
    return this.monteCarloEngine.simulate(
      this.currentState.probability,
      payoff,
      opts
    );
  ...
  /**
   * Assess risk across current pathways using Monte Carlo sampling.
   * @param {number} [samplesPerPathway=10000] – Samples for each pathway
   * @returns {Promise<Object>} Risk assessment with VaR, CVaR, percentiles
   */
  async assessPathwayRisk(samplesPerPathway = 10_000) {
    if (!this.currentState?.pathways) {
      throw new Error("Quantum space not initialized");
    }
    const pathways = this.currentState.pathways instanceof Map
      ? Array.from(this.currentState.pathways.values())
      : Array.isArray(this.currentState.pathways)
        ? this.currentState.pathways
        : [];
    return this.monteCarloEngine.assessRisk(pathways, samplesPerPathway);
  }

  /**
   * Perform sensitivity analysis on the quantum probability space.
   * @param {Function} payoff – Payoff function
   * @param {number}   [n]    – Samples per parameter
   * @returns {Promise<Object>} Sobol-like sensitivity indices
   */
  async sensitivityAnalysis(payoff, n = 5_000) {
    if (!this.currentState) {
      throw new Error("Quantum space not initialized");
    }
    return this.monteCarloEngine.sensitivityAnalysis(
      this.currentState.probability,
      payoff,
      n
    );
  ...
  /**
   * Run Monte Carlo Tree Search for optimal decision sequence.
   * @param {Function} expandFn   – (state) → Array<{state, action, prior}>
   * @param {Function} rolloutFn  – (state) → number (reward ∈ [0,1])
   * @param {Object}   [opts]     – MCTS overrides
   * @returns {Promise<Object>} Best action sequence with statistics
   */
  async mctsDecisionSearch(expandFn, rolloutFn, opts = {}) {
    if (!this.currentState) {
      throw new Error("Quantum space not initialized");
    }
    return this.monteCarloEngine.mcts(
      this.currentState.probability,
      expandFn,
      rolloutFn,
      opts
  ...
  /**
   * Get Monte Carlo engine diagnostics.
   * @returns {Object} Engine statistics
   */
  getMonteCarloStats() {
    return this.monteCarloEngine.getStats();
  }
...
export { SamplingStrategy, ConvergenceCriterion };
