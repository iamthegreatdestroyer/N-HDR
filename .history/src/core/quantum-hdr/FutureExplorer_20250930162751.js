...
class FutureExplorer {
  ...
  async _exploreBranch(branch, parameters) {
    ...
    for (let step = 0; step < this.explorationDepth; step++) {
      const timepoint = await this._evolveState(currentState, step, parameters);
  ...
  async _evolveState(state, step, parameters) {
    ...
    const interference = await this._calculateInterference(
      evolution,
      parameters
    );
  ...
  async _calculatePathConfidence(path) {
    ...
    const coherence =
      path.timeline.length > 0
        ? path.timeline[path.timeline.length - 1].state.coherence
        : 0;
  ...
  _updateHistory(paths) {
    this.explorationHistory.push({
      ...
      averageStability:
        paths.reduce((sum, p) => sum + p.stability, 0) / paths.length,
    });
  }
  ...
  _calculateAveragePathCount() {
    ...
    return (
      this.explorationHistory.reduce((sum, h) => sum + h.pathCount, 0) /
