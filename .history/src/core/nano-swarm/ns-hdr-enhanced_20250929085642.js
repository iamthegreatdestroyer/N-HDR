...
class ThermalManager {
  ...
  activateCooling() {
    if (!this.coolingActive && this.currentTemp > this.tempThreshold) {
      console.log(`Activating quantum cooling system at ${this.currentTemp.toFixed(1)}°C`);
  ...
  deactivateCooling() {
    if (this.coolingActive && this.currentTemp < this.tempThreshold - 10) {
      console.log(`Deactivating quantum cooling system at ${this.currentTemp.toFixed(1)}°C`);
      this.coolingActive = false;
    }
  }
...
class QuantumEntropyGenerator {
  ...
  getEntropy(bytes) {
    ...
    const entropy = this.entropyPool.slice(this.entropyIndex, this.entropyIndex + bytes);
