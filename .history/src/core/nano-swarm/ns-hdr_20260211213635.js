...
class NanoSwarmHDR {
  ...
  // ─────────────────────────────────────────────────────────────
  //  Claude-Flow Multi-Agent Orchestration (Phase 9.3)
  // ─────────────────────────────────────────────────────────────

  /**
   * Orchestrate a multi-agent workflow using swarm-accelerated task routing.
   *
   * @param {Object}   spec
   * @param {string}   spec.name        Workflow name
   * @param {string}   [spec.strategy]  One of Strategy values (parallel, pipeline, map_reduce, consensus, hierarchical)
   * @param {Object[]} spec.steps       Array of task specs ({ title, description, input, requiredRole, priority })
   * @param {Function} [spec.reducer]   For map_reduce strategy
   * @param {Function} [spec.voter]     For consensus strategy
   * @returns {Promise<Object>} Workflow results
   */
  async orchestrate(spec) {
    // Ensure orchestrator is connected to this swarm
    if (!this.orchestrator._swarmHDR) {
      this.orchestrator.connectSwarm(this);
    }

    console.log(`[Claude-Flow] Starting workflow: ${spec.name} (${spec.strategy ?? 'parallel'})`);

    const result = await this.orchestrator.executeWorkflow(spec);

    console.log(`[Claude-Flow] Workflow ${spec.name} completed`);
    return result;
  }
   ...
   * @param {string}   descriptor.role       AgentRole value
  ...
  registerAgent(descriptor) {
    return this.orchestrator.registerAgent(descriptor);
  }

  /**
   * Execute a single task through the orchestrator's agent pool.
   *
   * @param {Object}  spec  Same shape as orchestrator.createTask
  ...
  async executeTask(spec) {
    ...
    const taskId = this.orchestrator.createTask(spec);
    const completed = await this.orchestrator.awaitTask(taskId);
    return completed.result;
  }

  /**
   * Get orchestrator status including agent pool health.
   * @returns {Object}
   */
...
export { ClaudeFlowOrchestrator, createHDROrchestrator, Strategy, AgentRole };
