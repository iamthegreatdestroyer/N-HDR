...
class ClaudeFlowOrchestrator extends EventEmitter {
  ...
  getStatus() {
    const agents = Array.from(this.agents.values()).map((a) => ({
      ...
      avgLatencyMs: a.totalTasks > 0
        ? Math.round(a.totalLatencyMs / a.totalTasks)
        : 0,
    }));
  ...
  _schedule() {
    while (this.taskQueue.length > 0 && this.activeCount < this.maxConcurrentAgents) {
  ...
  async _runPipeline(workflow, spec) {
    ...
    for (const step of spec.steps) {
      ...
      pipeInput = typeof completed.result === "object"
        ? completed.result
