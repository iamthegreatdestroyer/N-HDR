...
class NanoSwarmHDR {
  ...
  _initializeQuantumState(state) {
    return {
      superposition: Array(config.quantum.dimensions).fill(0)
        .map(() => Math.random()),
  ...
  _combineQuantumStates(state1, state2) {
    ...
    return {
      superposition: state1.superposition.map((amp, i) => 
        (amp + state2.superposition[i]) / 2
      ),
      phase: (state1.phase + state2.phase) / 2,
      amplitude: Math.sqrt(
        (state1.amplitude ** 2 + state2.amplitude ** 2) / 2
      ),
    };
  }
  ...
  _applyCNOTGate(state, targets) {
    ...
    if (state.superposition[control] > 0.7) {  // Threshold for control qubit
  ...
  _applyPhaseGate(state, targets, angle) {
    ...
    for (const target of targets) {
      ...
      newState.superposition[target] = amplitude * Math.exp(1i * angle);
    }
  ...
  _applyToffoliGate(state, targets) {
    ...
    if (state.superposition[control1] > 0.7 && 
  ...
  _collapseToGroupState(particleState, groupState, coherence) {
    return {
      ...particleState,
      superposition: particleState.superposition.map((amp, i) => 
        (amp + groupState.superposition[i]) / 2 * coherence
      ),
