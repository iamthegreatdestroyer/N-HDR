...
class UpdateManager {
    ...
    async queueUpdate(version) {
        try {
            ...
            if (this.options.verifySignatures && !this._verifyUpdateSignature(update)) {
     ...
     */
    getUpdateHistory() {
        return Array.from(this._updateHistory.entries()).map(([version, details]) => ({
            version,
            ...details
        }));
    }
    ...
    async _verifySystemState() {
        ...
        if (!integrityCheck.valid) {
            throw new Error(`System integrity check failed: ${integrityCheck.reason}`);
        }
    ...
    async _loadManifest() {
        try {
            const manifestPath = path.join(this.options.updateDir, this.options.manifestFile);
