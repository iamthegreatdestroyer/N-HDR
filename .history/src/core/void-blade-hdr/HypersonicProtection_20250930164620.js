...
class HypersonicProtection {
    ...
    async reinforcePerimeter(perimeter) {
        try {
            ...
            await Promise.all(Array.from(this.resonators).map(async resonator => {
    ...
    async planDefense(target, zone) {
        ...
        const resonance = await this._generateResonancePattern(frequency);
        const effectiveness = await this._calculateEffectiveness(
            resonance,
            target
        );
    ...
    async _calibrateFrequencies() {
        const frequencies = [];
        for (let f = this.modulation.range[0]; f <= this.modulation.range[1]; f += this.modulation.step) {
            frequencies.push(f);
        }
    ...
    async _calculateEffectiveness(resonance, target) {
        ...
        score += weights.frequency * (1 - Math.abs(
            target.frequency - resonance.fundamental
        ) / target.frequency);

        const harmonicMatch = resonance.harmonics.reduce((sum, h) =>
            sum + (1 - Math.abs(h - target.frequency) / h),
            0
        ) / resonance.harmonics.length;
        score += weights.harmonics * harmonicMatch;

        const phaseMatch = resonance.phases.reduce((sum, p) =>
            sum + Math.cos(p - target.phase),
            0
    ...
    async _alignResonator(resonator, field) {
        const pattern = field.pattern.find(p =>
            Math.abs(p.frequency - resonator.frequency) < this.modulation.step
        );
    ...
    async _recalibrateBarriers() {
        for (const [id, barrier] of this.barriers) {
            const resonance = await this._generateResonancePattern(
                barrier.frequency
            );
            const field = await this._generateBarrierField(resonance);
    ...
    async _synchronizeBarriers() {
        ...
        const averageFrequency = barriers.reduce(
            (sum, b) => sum + b.frequency,
            0
        ) / barriers.length;
