...
const HypersonicProtection = require('./HypersonicProtection');
const QuantumFieldDistortion = require('./QuantumFieldDistortion');
const PerceptionNullifier = require('./PerceptionNullifier');
const SelectiveTargeting = require('./SelectiveTargeting');
...
class VoidBladeHDR {
    ...
    async activateBarrier(zoneId, parameters) {
        ...
        try {
            ...
            const field = await this.quantumDistortion.amplifyField(
                barrier.field
            );
    ...
    async _initializeZoneDefenses(config) {
        ...
        if (config.hypersonic) {
            defenses.push(await this.hypersonicProtection.initializeDefense(
                config.hypersonic
            ));
        }

        if (config.quantum) {
            defenses.push(await this.quantumDistortion.initializeDefense(
                config.quantum
            ));
        }

        if (config.perception) {
            defenses.push(await this.perceptionNullifier.initializeDefense(
                config.perception
            ));
        }
    ...
    async _activateZoneDefenses(zone) {
        await Promise.all(zone.defenses.map(async defense => {
