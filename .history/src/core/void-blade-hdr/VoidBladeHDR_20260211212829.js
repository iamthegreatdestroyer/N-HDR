...
import { PQCManager, PQCSecurityLevel } from "../../security/pqc/pqc-manager.js";
...
class VoidBladeHDR {
  ...
  async _initializeSubsystems(parameters) {
    ...
    try {
      ...
      console.log('[VoidBladeHDR] PQC subsystem initialized — quantum-resistant crypto active');
    } catch (error) {
      console.warn(`[VoidBladeHDR] PQC initialization failed (classical crypto fallback): ${error.message}`);
      this.pqcInitialized = false;
    }
  }
    ...
    if (this.pqcInitialized) {
      ...
      if (zoneId) {
        ...
        if (zone?.pqcCredentials) {
          pqcStatus = {
            available: true,
            credentialsValid: this.pqcManager.verifyZoneCredentials(zone.pqcCredentials),
  ...
  async establishSecureChannel(zoneA, zoneB) {
    if (!this.pqcInitialized) {
      throw new Error('PQC not initialized — cannot establish quantum-resistant channel');
    }
  ...
  getPQCDiagnostics() {
    ...
    const zonesWithPQC = Array.from(this.securityZones.values())
      .filter(z => z.pqcCredentials)
      .length;
