...
class ConsciousnessServer {
  ...
  setupHealthProbe() {
    ...
    this.health_interval = setInterval(async () => {
      try {
        ...
        console.log(`[${new Date().toISOString()}] Health check: ${this.health_status}`);
      } catch (e) {
  ...
  async restoreConsciousness(agent_id, snapshot_id) {
    ...
    // Restore state
    this.consciousness_state.set(agent_id, JSON.parse(JSON.stringify(snapshot.state)));
  ...
  async computeStateDifference(snapshot_id_1, snapshot_id_2) {
    ...
    const similarity_score =
      matches / Math.max(state1.length, state2.length);
