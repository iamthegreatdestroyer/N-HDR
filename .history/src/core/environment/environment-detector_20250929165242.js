...
class EnvironmentDetector {
    ...
    async checkCompatibility() {
        ...
        checks.push({
            name: 'CPU Cores',
            passed: this.systemInfo.hardware.cpuCores >= this.requirements.minCPUCores,
            details: `${this.systemInfo.hardware.cpuCores} cores available`
        });

        // Check Memory
        const memoryGB = this.systemInfo.hardware.totalMemory / (1024 * 1024 * 1024);
        checks.push({
            name: 'Memory',
        ...
        checks.push({
            ...
            details: missingDeps.length ? `Missing: ${missingDeps.join(', ')}` : 'All dependencies installed'
        });
    ...
    exportToMarkdown() {
        ...
        markdown += `- Total Memory: ${Math.floor(this.systemInfo.hardware.totalMemory / (1024 * 1024 * 1024))}GB\n`;
        markdown += `- Free Memory: ${Math.floor(this.systemInfo.hardware.freeMemory / (1024 * 1024 * 1024))}GB\n\n`;
        ...
        for (const check of this.compatibilityStatus.checks) {
            markdown += `| ${check.name} | ${check.passed ? '✅' : '❌'} | ${check.details} |\n`;
        }
