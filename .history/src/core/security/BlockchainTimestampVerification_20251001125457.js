...
class BlockchainTimestampVerification extends EventEmitter {
   ...
   */
  async hashDirectory(directoryPath, extensions = ['.js', '.ts', '.jsx', '.tsx', '.json']) {
    const results = [];
  ...
  buildMerkleTree(hashInfos) {
    ...
    while (currentLevel.length > 1) {
      ...
      for (let i = 0; i < currentLevel.length; i += 2) {
        ...
        const combinedHash = crypto.createHash('sha3-512')
  ...
  async generateVerificationReport() {
    const reportPath = path.join(this.config.timestampDirectory, 'VERIFICATION-REPORT.md');
...
${Array.from(this.timestamps.values()).map(ts => `
### Timestamp: ${ts.date}
...
${Array.from(this.fileHashes.values()).slice(0, 10).map(info => `
...
including patent-pending technologies, are reserved by ${this.config.authorName}.
...
export async function generateTimestampVerificationCLI(sourceDir, outputDir) {
  ...
  verifier.on('merkle-tree-built', (info) => {
    console.log(`  🌳 Merkle tree: ${info.leafCount} leaves, depth ${info.treeDepth}`);
  });
