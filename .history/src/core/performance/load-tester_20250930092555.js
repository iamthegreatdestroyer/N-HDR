...
class LoadTester {
    ...
    async _executeWarmup(config) {
        ...
        while (Date.now() - warmupStart < config.warmup) {
            ...
            currentLoad = (Date.now() - warmupStart) / config.warmup * config.targetLoad * 0.5;
    ...
    async _executeCooldown(config) {
        ...
        while (Date.now() - cooldownStart < config.cooldown) {
            ...
            currentLoad = (1 - (Date.now() - cooldownStart) / config.cooldown) * config.targetLoad;
            await this._generateLoad(config.id, currentLoad);
            await new Promise(resolve => setTimeout(resolve, 1000));
        }
    }
    ...
    _collectMetrics(testId) {
        ...
        this.metrics.recordMetric('loadTest.cpuUsage', resourceUsage.cpu.user / 1000000);
        this.metrics.recordMetric('loadTest.memoryUsage', resourceUsage.memory.heapUsed);
    }
