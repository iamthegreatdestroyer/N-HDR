...
class OptimizationEngine {
    ...
    _generateCandidates() {
        ...
        for (let i = 0; i < numCandidates; i++) {
            ...
            for (const [param, bounds] of Object.entries(this.options.parameterBounds)) {
                ...
                if (Math.random() < this.options.explorationRate) {
                    ...
                    // Local optimization
                    const delta = (Math.random() - 0.5) * 2 * range * this.options.learningRate;
                    newValue = currentValue + delta;
                }

                // Clamp to bounds
                newValue = Math.max(bounds.min, Math.min(bounds.max, Math.floor(newValue)));
    ...
    _calculateLatency(analysis) {
        ...
        return metrics.responseTimes ? 
            this._calculatePercentile(metrics.responseTimes, 95) : 0;
    }
    ...
    _calculateResourceUtilization(analysis) {
        ...
        const memory = metrics.memory ? 
            (metrics.memory.heapUsed / metrics.memory.heapTotal) : 0;
    ...
    _calculateImprovements() {
        ...
        return Array.from(current.entries()).reduce((total, [metric, value]) => {
    ...
    _checkConvergence(previousState) {
        ...
        const avgImprovement = recentImprovements.reduce((a, b) => a + b, 0) / 
     ...
     */
    _updateMetrics() {
        this.metrics.recordMetric('optimization.iteration', 
            this.currentState.iteration);

        this.metrics.recordMetric('optimization.improvement',
