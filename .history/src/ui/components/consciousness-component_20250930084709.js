...
class ConsciousnessComponent {
  ...
  _checkSynchronization() {
    ...
    this._data.synchronization = (synchronized / this._data.layers.length) * 100;
  }
  ...
  _prepareVisualizationData() {
    return {
      ...
      layers: this._data.layers.map(layer => {
        ...
        return {
          id: layer.id,
          color: this.options.layerColors[layer.index % this.options.layerColors.length],
  ...
  _calculateAverageIntegrity() {
    ...
    const sum = Array.from(this._data.integrityScores.values())
      .reduce((a, b) => a + b, 0);
