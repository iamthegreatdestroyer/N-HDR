...
class SwarmComponent {
  ...
  _updateNodesStatus() {
    ...
    this._data.nodes.forEach((node, nodeId) => {
      ...
      // Update node metrics
      if (this.dashboard.controller?.swarmManager) {
        const metrics = this.dashboard.controller.swarmManager
  ...
  _updateTopology() {
    ...
    if (now - this._lastUpdate >= this.options.topologyUpdateInterval) {
      this._data.topology = this.dashboard.controller.swarmManager.getTopology();
  ...
  _processOptimizationQueue() {
    ...
    if (this._optimizationQueue.length === 0 &&
  ...
  _updateMetrics() {
    ...
    this._data.metrics.activeNodes = Array.from(this._data.nodes.values())
  ...
  _calculateLoadBalance() {
    ...
    const loads = Array.from(this._data.nodes.values())
      .map(node => node.metrics.load || 0);
