...
class ThermalComponent {
  ...
  async _initializeData() {
    ...
    ['cpu', 'memory', 'system'].forEach(type => {
      this._data.history[type] = Array(this.options.historyLength)
        .fill(initialData);
    });

    // Get initial thermal zones if available
    if (this.dashboard.controller?.thermalManager) {
      this._data.zones = await this.dashboard.controller.thermalManager
        .getThermalZones();
    }
  ...
  _updatePredictions() {
    ...
    ['cpu', 'memory', 'system'].forEach(type => {
      ...
      if (history.length >= 60) { // Need at least 1 minute of data
