...
class MemoryBenchmark {
    ...
    async benchmarkFragmentation() {
        const { FragmentationAnalyzer } = require('../core/memory/fragmentation-analyzer');
        ...
        // Calculate summary metrics
        results.metrics = {
            averageExternalFragmentation: results.tests.reduce((sum, test) => 
                sum + test.measurements[0].externalFragmentation, 0) / results.tests.length,
            averageInternalFragmentation: results.tests.reduce((sum, test) => 
                sum + test.measurements[0].internalFragmentation, 0) / results.tests.length
        };
    ...
    async benchmarkPersistence() {
        const { PersistenceManager } = require('../core/memory/persistence-manager');
        ...
        // Calculate summary metrics
        results.metrics = {
            averageWriteSpeed: results.tests.reduce((sum, test) => 
                sum + test.measurements[0].writeSpeed, 0) / results.tests.length,
            averageReadSpeed: results.tests.reduce((sum, test) => 
                sum + test.measurements[0].readSpeed, 0) / results.tests.length
        };
