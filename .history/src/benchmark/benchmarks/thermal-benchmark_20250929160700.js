...
class ThermalBenchmark {
    ...
    async benchmarkAdaptiveThrottling() {
        ...
        results.throttlingEffectiveness = this._calculateThrottlingEffectiveness(samples);
    ...
    async benchmarkPredictiveCooling() {
        ...
        results.coolingEfficiency = this._calculatePredictiveCoolingEfficiency(samples);
    ...
    _calculateStabilizationTime(samples) {
        ...
        for (let i = windowSize; i < samples.length; i++) {
            ...
            const stable = window.every(s => Math.abs(s.temp - avgTemp) <= tolerance);
    ...
    _calculateAverageResponseTime(samples) {
        ...
        return changes.length > 0 ? 
            changes.reduce((sum, time) => sum + time, 0) / changes.length : 0;
    }
    ...
    _calculatePowerSavings(samples) {
        ...
        const actualPower = samples.reduce((sum, s) => sum + s.powerUsage, 0) / samples.length;
    ...
    _calculatePerformanceImpact(samples) {
        const targetPerformance = samples.reduce((sum, s) => sum + s.load, 0) / samples.length;
        const actualPerformance = samples.reduce((sum, s) => sum + s.performance, 0) / samples.length;
    ...
    _calculateAdaptationSpeed(samples) {
        ...
        return adaptationPoints.length > 0 ?
            adaptationPoints.reduce((sum, time) => sum + time, 0) / adaptationPoints.length : 0;
    }
    ...
    _calculateTemperatureStability(samples) {
        const temperatures = samples.map(s => s.actual).filter(t => t !== undefined);
