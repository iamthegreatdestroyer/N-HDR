...
class SecurityBenchmark {
    ...
    async benchmarkEncryption() {
        ...
        results.metrics = {
            averageEncryptionSpeed: results.tests.reduce((sum, test) => 
                sum + test.measurements[0].encryptionSpeed, 0) / results.tests.length,
            averageDecryptionSpeed: results.tests.reduce((sum, test) => 
                sum + test.measurements[0].decryptionSpeed, 0) / results.tests.length
        };
    ...
    async benchmarkAuthentication() {
        ...
        // Calculate summary metrics
        results.metrics = {
            averageSuccessRate: results.tests.reduce((sum, test) => 
                sum + test.measurements[0].successRate, 0) / results.tests.length,
    ...
    async benchmarkIntegrityChecks() {
        ...
        // Calculate summary metrics
        results.metrics = {
            averageVerificationSpeed: results.tests.reduce((sum, test) => 
                sum + test.measurements[0].verificationSpeed, 0) / results.tests.length,
            averageDetectionRate: results.tests.reduce((sum, test) => 
                sum + test.measurements[0].detectionRate, 0) / results.tests.length
        };
    ...
    async benchmarkQuantumResistance() {
        const { QuantumSecurityManager } = require('../core/security/quantum-security-manager');
        ...
        // Calculate summary metrics
        results.metrics = {
            averageKeyGenSpeed: results.tests.reduce((sum, test) => 
                sum + test.measurements[0].keyGenSpeed, 0) / results.tests.length,
            averageEncapsulationSpeed: results.tests.reduce((sum, test) => 
                sum + test.measurements[0].encapsulationSpeed, 0) / results.tests.length
        };
