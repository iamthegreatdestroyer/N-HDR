...
class PerformanceBenchmark extends EventEmitter {
  ...
  async runBenchmarkSuite(benchmarks) {
    ...
    for (const [name, benchmark] of Object.entries(benchmarks)) {
      ...
      if (target) {
        ...
        console.log(
          `Target: ${target}ms, Actual: ${result.stats.mean.toFixed(2)}ms - ${
            passed ? "✓ PASS" : "✗ FAIL"
          }`
        );
      }
  ...
  _calculateImprovement(baseline, optimized) {
    const meanImprovement =
      ((baseline.stats.mean - optimized.stats.mean) / baseline.stats.mean) *
      100;
    const medianImprovement =
      ((baseline.stats.median - optimized.stats.median) /
        baseline.stats.median) *
      100;
    const p95Improvement =
      ((baseline.stats.p95 - optimized.stats.p95) / baseline.stats.p95) * 100;

    const memoryImprovement =
      baseline.memory && optimized.memory
        ? ((baseline.memory.delta.heapUsed - optimized.memory.delta.heapUsed) /
            baseline.memory.delta.heapUsed) *
          100
        : 0;
  ...
  _displayComparison(comparison) {
    console.log("\nResults:");
    console.log(
      `Baseline: ${comparison.baseline.stats.mean.toFixed(
        2
      )}ms (±${comparison.baseline.stats.stdDev.toFixed(2)}ms)`
    );
    console.log(
      `Optimized: ${comparison.optimized.stats.mean.toFixed(
        2
      )}ms (±${comparison.optimized.stats.stdDev.toFixed(2)}ms)`
    );
    console.log(
      `Improvement: ${comparison.improvement.mean.toFixed(2)}% ${
        comparison.improvement.faster ? "faster" : "slower"
      }`
    );

    if (comparison.improvement.memory !== 0) {
      console.log(
        `Memory: ${comparison.improvement.memory.toFixed(2)}% ${
          comparison.improvement.memory > 0 ? "less" : "more"
        }`
      );
    }
  }
  ...
  async exportMarkdown(filePath, results) {
    ...
    markdown += "## Results\n\n";
    markdown +=
      "| Benchmark | Mean | Median | P95 | P99 | Min | Max | Target | Status |\n";
    markdown +=
      "|-----------|------|--------|-----|-----|-----|-----|--------|--------|\n";
    ...
    for (const [name, result] of Object.entries(results.benchmarks)) {
      ...
      const status =
        target !== "N/A" && result.stats.mean <= target ? "✓" : "✗";
    ...
    if (this.options.memoryProfiling) {
      ...
      for (const [name, result] of Object.entries(results.benchmarks)) {
        if (result.memory) {
          const heapDelta = (
            result.memory.delta.heapUsed /
            1024 /
            1024
          ).toFixed(2);
          const externalDelta = (
            result.memory.delta.external /
            1024 /
            1024
          ).toFixed(2);
