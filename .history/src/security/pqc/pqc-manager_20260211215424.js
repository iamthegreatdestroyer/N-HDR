...
export class PQCManager {
  ...
  async initialize() {
    ...
    for (const level of Object.values(PQCSecurityLevel)) {
      try {
        ...
        console.error(
          `PQC level ${level} initialization failed:`,
          error.message,
        );
        throw new Error(
          `PQC initialization failed at level ${level}: ${error.message}`,
        );
      }
    }
  ...
  createZoneCredentials(zoneId, level) {
    ...
    const identitySignature = this.sign(
      zoneIdentity,
      dsaKeys.secretKey,
      secLevel,
    );
  ...
  verifyZoneCredentials(credentials) {
    try {
      const publicKey = new Uint8Array(
        Buffer.from(credentials.dsa.publicKey, "base64"),
      );
      const signature = new Uint8Array(
        Buffer.from(credentials.identity.signature, "base64"),
      );
