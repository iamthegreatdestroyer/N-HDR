...
class NHDRCLI {
    ...
    async initialize() {
        this.program
            .version('1.0.0')
            .description('N-HDR Command Line Interface');
    ...
    async handleDeploy(options) {
        ...
        try {
            if (options.environment) {
                ...
                spinner.succeed(`Deployment to ${options.environment} completed successfully`);
                ...
                spinner.succeed(`Rollback to ${options.rollback} completed successfully`);
            }
        } catch (error) {
