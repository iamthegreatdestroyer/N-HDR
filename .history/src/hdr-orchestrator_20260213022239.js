...
export class HDRIntegrationOrchestrator extends EventEmitter {
  ...
  publishAgentCard(agentCard) {
    try {
      ...
      if (!agentCard?.registered_at) {
        validation_errors.push("Agent card must have registered_at timestamp");
      }
  ...
  collectMetrics() {
    const metrics = {
      genesis: this.genesis
        ? {
            agentsBred: this.genesis.metrics?.populationSize || 0,
            populationSize: this.genesis.metrics?.populationSize || 0,
            averageFitness: this.genesis.metrics?.averageFitness || 0,
            diversity: this.genesis.metrics?.diversity || 0,
          }
        : {},
      oracle: this.oracle
        ? {
            predictionsGenerated: this.oracle.metrics?.predictionsGenerated || 0,
            cascadesComputed: this.oracle.metrics?.cascadesComputed || 0,
            accuracyScore: this.oracle.metrics?.accuracyScore || 0,
          }
        : {},
      diffusion: this.diffusion
        ? {
            consequencesExplored: this.diffusion.metrics?.consequencesExplored || 0,
            scenariosGenerated: this.diffusion.metrics?.scenariosGenerated || 0,
          }
        : {},
      integration: this.systemMetrics,
    };
    return metrics;
  }
