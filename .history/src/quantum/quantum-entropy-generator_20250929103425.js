...
class QuantumEntropyGenerator {
  ...
  constructor(options = {}) {
    ...
    this.refillTimer = setInterval(() => this._refillEntropyPool(), this.refillInterval);
  }
  ...
  getRandomBytes(size) {
    ...
    const bytes = this.entropyPool.slice(this.entropyOffset, this.entropyOffset + size);
  ...
  _addHardwareEntropy(buffer) {
    ...
    if (this.hardwareSources.includes('cpu_timing')) {
      metrics.push(...os.cpus().map(cpu => cpu.times.user + cpu.times.nice + cpu.times.sys));
    }
