...
class ForgeHDRCore extends EventEmitter {
   ...
   */
  initializeMetrics() {
    this.prometheus.defineCounter('forge_optimization_count', 
      'Total optimizations executed by FORGE-HDR')
    
    this.prometheus.defineGauge('forge_cost_saved_dollars',
      'Total cloud costs saved by FORGE-HDR optimizations')
    
    this.prometheus.defineGauge('forge_topology_stability',
      'Current topology stability score (0-100)')
    
    this.prometheus.defineCounter('forge_decision_rollbacks',
      'Optimizations that required rollback')
    
    this.prometheus.defineCounter('forge_cascade_prevention_count',
      'Cascading failures prevented by FORGE-HDR')
    
    this.prometheus.defineHistogram('forge_optimization_latency_ms',
      'Time to execute topology optimization')
    
    this.prometheus.defineHistogram('forge_analysis_latency_ms',
  ...
  async analyze() {
    ...
    try {
      ...
      this.prometheus.recordHistogram('forge_analysis_latency_ms', 
        Date.now() - startTime)
  ...
  isSafeToExecute(proposal, currentTopology) {
    ...
    if (projectedDelta.cpuIncrease > this.config.safetyThresholds.maxCpuIncrease) {
    ...
    if (proposal.affectedServices?.some(s => s.criticality === 'CRITICAL' && s.projectedPerfDegradation > 5)) {
  ...
  estimateAvailability(proposal) {
    ...
    const currentAvailability = this.metrics.metrics.system_availability_percent || 99.9
  ...
  async executeOptimization(proposal, previousTopology) {
    ...
    try {
      ...
      if (proposal.projectedSavings) {
        this.prometheus.gaugeSet('forge_cost_saved_dollars',
          (this.costTracker.totalSavings += proposal.projectedSavings))
      }

      this.prometheus.recordHistogram('forge_optimization_latency_ms',
      ...
      console.error(`[FORGE-HDR] Optimization ${optimizationId} failed:`, error)
  ...
  async verifyOptimization(proposal) {
    ...
    const errorRate = this.metrics.metrics.http_requests_error_rate || 0
    if (errorRate > 2) {
      throw new Error(`Topology verification failed: error rate ${errorRate}% > threshold`)
    }
  ...
  topologiesMatch(topology1, topology2) {
    return topology1?.pods?.length === topology2?.pods?.length &&
  ...
  calculateStability(metrics) {
    const errorRateScore = Math.max(0, 100 - (metrics.http_requests_error_rate || 0) * 10)
    const availabilityScore = metrics.system_availability_percent || 100
    const latencyScore = Math.max(0, 100 - ((metrics.http_request_latency_p99_ms || 0) / 5))
