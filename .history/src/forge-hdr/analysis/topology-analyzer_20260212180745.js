...
class TopologyAnalyzer extends EventEmitter {
  ...
  async analyzeTopology() {
    try {
      ...
      const rawTopology = await this.config.kubernetesClient.getCurrentTopology()
  ...
  findCriticalPaths() {
    ...
    for (const [pod, deps] of this.dependencies) {
      ...
      if (path.length > 1) {  // Only consider paths with actual dependencies
        paths.push({
          startPod: pod,
          path,
          length: path.length,
  ...
  identifyBottlenecks() {
    ...
    for (const [pod, count] of connectionCount) {
      if (count >= 5) {  // Pod is depended on by 5+ other pods
        bottlenecks.push({
          pod,
  ...
  getSummary() {
    return {
      ...
      dependencies: {
        ...
        avgConnectionsPerPod: this.topology.pods.length > 0 ? 
          Array.from(this.dependencies.values()).reduce((sum, deps) => sum + deps.length, 0) / this.topology.pods.length : 0
      },
      criticalPaths: {
