...
class KubernetesClient {
  ...
  initializeClient() {
    ...
    try {
      ...
      console.warn('Failed to load kubeconfig, using mock client:', error.message)
  ...
  async getCurrentTopology() {
    try {
      const deployments = await this.appsApi.listNamespacedDeployment(this.config.namespace)
  ...
  parsePods(items) {
    return items.map(pod => ({
      ...
      restartCount: pod.status.containerStatuses?.reduce((sum, c) => sum + (c.restartCount || 0), 0) || 0
    }))
  }
  ...
  summarizeTopology(depResponse, podResponse, nodeResponse) {
    return {
      ...
      readyPods: podResponse.body.items?.filter(p => p.status.phase === 'Running').length || 0,
      pendingPods: podResponse.body.items?.filter(p => p.status.phase === 'Pending').length || 0,
      failedPods: podResponse.body.items?.filter(p => p.status.phase === 'Failed').length || 0
    }
  }
  ...
  async applyOptimization(proposal) {
    try {
      ...
      if (proposal.changes.istioVirtualService) {
        await this.applyIstioChange(proposal.changes.istioVirtualService, 'VirtualService')
      }

      if (proposal.changes.istioDestinationRule) {
        await this.applyIstioChange(proposal.changes.istioDestinationRule, 'DestinationRule')
      }
  ...
  async applyDeploymentChange(deploymentChange) {
    const name = deploymentChange.kind === 'Deployment' ? 'primary' : deploymentChange.name
    ...
    try {
      ...
      console.warn(`Warning: Could not patch deployment ${name}:`, error.message)
  ...
  async verifyTopologyStability(maxWait = 30000) {
    ...
    while (Date.now() - startTime < maxWait) {
      ...
      const allStable = topology.deployments.every(dep => 
  ...
  compareTopologies(topology1, topology2) {
    ...
    for (const [name, dep1] of deps1) {
      ...
      if (!dep2) {
        ...
        differences.push(`Deployment ${name}: replicas changed from ${dep1.replicas.desired} to ${dep2.replicas.desired}`)
      }
    }
