...
class PrometheusClient {
  ...
  parseMetricsResponse(data) {
    ...
    data.data.result.forEach((result) => {
      ...
      if (metric.__name__ === "rate(requests_total)") {
        metrics.network.requestRate =
          (metrics.network.requestRate || 0) + value;
      }
    ...
    metrics.memory.total = Object.values(metrics.memory).reduce(
      (a, b) => a + b,
      0,
    );
    metrics.memory.average =
      metrics.memory.total / Object.keys(metrics.memory).length;
  ...
  recordCostSaving(type, amount) {
    this.metrics.costSaved
      .labels(type)
      .set(
        (this.metrics.costSaved.labels(type).get()?.values[0]?.value || 0) +
          amount,
      );
  }
  ...
  getMetricsSummary(minutes = 60) {
    ...
    const recentHistory = this.metricsHistory.filter(
    ...
    return {
      ...
      memoryAverage: this.calculateAverage(
        recentHistory,
        "metrics.memory.average",
      ),
      memoryMax: this.calculateMax(recentHistory, "metrics.memory.total"),
      networkAverage: this.calculateAverage(
        recentHistory,
        "metrics.network.requestRate",
      ),
  ...
  calculateAverage(history, path) {
    const values = history
      .map((entry) => this.getFieldValue(entry, path))
      .filter((v) => v > 0);
