...
class TopologyOptimizer {
  ...
  generateOptimization(data) {
    ...
    switch (selectedOpportunity.type) {
      case 'OVER_PROVISION_CPU':
        proposal = this.generateScaleDownProposal(selectedOpportunity, dna, topology)
        break
      
      case 'INSUFFICIENT_SCALING':
        proposal = this.generateScaleUpProposal(selectedOpportunity, dna, topology)
        break
      
      case 'CASCADE_PREVENTION':
        proposal = this.generateCascadePreventionProposal(selectedOpportunity, dna, topology)
        break
      
      case 'NODE_AFFINITY':
        proposal = this.generateAffinityOptimiationProposal(selectedOpportunity, dna, topology)
        break
      
      case 'MEMORY_CONTENTION':
        proposal = this.generateMemoryOptimizationProposal(selectedOpportunity, dna, topology)
        break
    ...
    // Add safety checks
    proposal.safetyChecks = this.performSafetyAnalysis(proposal, topology, constraints)
  ...
  generateScaleDownProposal(opportunity, dna, topology) {
    ...
    return {
      ...
      projectedResourceDelta: {
        cpuIncrease: -((currentReplicas - targetReplicas) / currentReplicas) * 100,
        memoryIncrease: 0,
        cpuReduction: ((currentReplicas - targetReplicas) / currentReplicas) * 100
      },
  ...
  generateScaleUpProposal(opportunity, dna, topology) {
    ...
    return {
      ...
      impact: {
        costIncrease: ((targetReplicas - currentReplicas) / currentReplicas) * 20,  // Rough estimate
        latencyReduction: 15,  // ms
        availabilityImprovement: 0.5  // %
      },

      projectedResourceDelta: {
        cpuIncrease: ((targetReplicas - currentReplicas) / currentReplicas) * 100,
        memoryIncrease: ((targetReplicas - currentReplicas) / currentReplicas) * 100
      },
      ...
      projectedSavings: -((targetReplicas - currentReplicas) / currentReplicas) * 30,  // Cost increase
  ...
  analyzeResilience(proposal, topology) {
    return {
      ...
      totalCapacityChange: (proposal.projectedResourceDelta?.cpuIncrease || 0) +
