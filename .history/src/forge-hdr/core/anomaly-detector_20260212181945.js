...
class AnomalyDetector {
  ...
  async detectAnomalies(metricName, value, timestamp = Date.now()) {
    ...
    try {
      ...
      this.metrics.averageDetectionTime = this.metrics.totalDetectionTime / (this.metrics.anomaliesDetected + 1)
      ...
      return { isAnomaly: false, confidence: 0, reason: 'detection_error', error: error.message }
    }
  }
  ...
  detectZScore(metricName, value) {
    ...
    return {
      ...
      mean,
      stdDev,
      anomalyType: zScore > threshold ? (value > mean ? 'spike' : 'dip') : 'normal'
    }
  }
  ...
  detectIQR(metricName, value) {
    ...
    return {
      ...
      lowerBound,
      upperBound,
      anomalyType: isAnomaly ? (value > upperBound ? 'spike' : 'dip') : 'normal'
    }
  }
  ...
  detectIsolationForest(metricName, value) {
    ...
    const nearestNeighborsCount = values.filter(v => 
  ...
  async detectProphet(metricName, value) {
    ...
    if (series.length < 20) {
      return { isAnomaly: false, confidence: 0, reason: 'insufficient_data_for_prophet' }
    }
    ...
    const residuals = values.slice(-10).map((v, i) => 
      Math.abs(v - (values[i - 1] || v))
    )
    ...
    const confidence = isAnomaly ?
      Math.min(Math.max(
        Math.abs(value - forecast) / Math.max(meanResidual, 1),
        0.5
      ), 1.0) : 0
  ...
  calculateSeasonality(values, period) {
    ...
    const previousMean = previousPeriod.reduce((a, b) => a + b, 0) / previousPeriod.length
