...
class ResourceOptimizer extends EventEmitter {
  ...
  async performOptimization() {
    try {
      ...
      this.recordOptimizationCycle({
        ...
        totalCostSavings: applied.reduce(
  ...
  async identifyOptimizations(topology, metrics) {
    ...
    opportunities.push(
      ...this.findResourceRequestMismatches(topology, metrics),
    );

    // Strategy 5: Vertical scaling
    opportunities.push(
      ...this.findVerticalScalingOpportunities(topology, metrics),
    );

    // Strategy 6: Node consolidation
    opportunities.push(
      ...this.findNodeConsolidationOpportunities(topology, metrics),
    );
    ...
    for (const pod of topology.pods) {
      ...
      if (cpuUsage < cpuRequest * 0.3 || memoryUsage < memoryRequest * 0.3) {
        ...
        opportunities.push({
          ...
          estimatedSavings:
            (cpuRequest - newCpu) * 30 + (memoryRequest - newMemory) * 0.05,
          confidence: 0.9,
          riskLevel: "low",
        });
      }
    ...
    for (const opportunity of rankedOptimizations) {
      try {
        ...
        if (this.config.circuitBreaker) {
          await this.config.circuitBreaker.execute(() =>
  ...
  async scaleHorizontally(opportunity) {
    console.log(
      `Scaling ${opportunity.deployment} to ${opportunity.recommendedReplicas} replicas`,
    );
    // Would update deployment replica count
  }
  ...
  async optimizeNetwork(opportunity) {
    console.log(
      `Optimizing network for ${opportunity.bottleneckCount} bottlenecks`,
    );
  ...
  calculateImprovements(applied) {
    return {
      ...
      avgConfidence:
        applied.length > 0
          ? applied.reduce((sum, opt) => sum + opt.confidence, 0) /
            applied.length
          : 0,
    };
  }
