...
class LoadBalancer extends EventEmitter {
  ...
  selectLatencyOptimized(pods) {
    return pods.reduce((best, current) => {
      return (current.latency || Infinity) < (best.latency || Infinity) ? current : best
    })
  }
  ...
  recordRequest(podName, duration, success = true, errorRate = 0) {
    const pod = Array.from(this.podMetrics.values()).find(p => p.podName === podName)
    if (!pod) return
  ...
  getPodStatus(podName) {
    if (podName) {
      return Array.from(this.podMetrics.values()).find(p => p.podName === podName)
    }
  ...
  getServiceStatus(service, namespace = 'default') {
    ...
    const avgLatency = pods.reduce((sum, p) => sum + p.latency, 0) / pods.length
    const avgErrorRate = pods.reduce((sum, p) => sum + p.errorRate, 0) / pods.length
    ...
    return {
      ...
      healthyPods: healthy,
      healthPercentage: pods.length > 0 ? (healthy / pods.length * 100).toFixed(2) : 0,
      totalRequests,
   ...
   */
  getStatistics() {
    const avgLatency = this.metrics.requestsRouted > 0 ? 
      (this.metrics.totalLatency / this.metrics.successfulRoutes).toFixed(2) : 0

    const successRate = this.metrics.requestsRouted > 0 ?
      (this.metrics.successfulRoutes / this.metrics.requestsRouted * 100).toFixed(2) : 0
    ...
    return {
      ...
      podMetrics: {
        ...
        healthyPods: Array.from(this.podMetrics.values()).filter(p => p.healthy).length,
