  ...
  getBucketStatus(serviceName) {
    if (serviceName && this.buckets.has(serviceName)) {
      ...
      return {
        ...
        utilizationRate:
          bucket.accepted / (bucket.accepted + bucket.rejected || 1),
      };
    }
    ...
    return {
      ...
      utilizationRate:
        this.globalBucket.accepted /
        (this.globalBucket.accepted + this.globalBucket.rejected || 1),
    };
  }
  ...
  getCurrentRate() {
    ...
    const acceptanceRate =
      recentRequests.length > 0
        ? ((accepted / recentRequests.length) * 100).toFixed(2)
        : 100;
      ...
      metrics: {
        ...
        acceptanceRate:
          this.metrics.totalRequests > 0
            ? (
                (this.metrics.acceptedRequests / this.metrics.totalRequests) *
                100
              ).toFixed(2)
            : 100,
      },
