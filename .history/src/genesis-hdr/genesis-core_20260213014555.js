...
class GenesisHDR extends EventEmitter {
  ...
  /**
   * Analyze population diversity metrics
   */
  getPopulationDiversity(population = []) {
    if (!population || population.length === 0) {
      return {
        phenotypicDiversity: 0,
        genetic_variance: 0,
        trait_distribution: 0,
        uniquePersonalities: 0,
        specializations: [],
        average_pairwise_distance: 0,
        averageFitness: 0,
      };
    }

    // Calculate genetic variance across agents
    const fitnessScores = population.map(a => a.fitnessScore || 0);
    const avgFitness = fitnessScores.reduce((a, b) => a + b, 0) / population.length;
    const geneticVariance = fitnessScores.reduce((sum, f) => sum + Math.pow(f - avgFitness, 2), 0) / population.length;

    // Count unique personalities
    const personalities = new Set(population.map(a => a.genes?.personality || "unknown"));
    
    // Count unique specializations
    const specializations = new Set(population.map(a => a.genes?.domainSpecialization || "unknown"));

    // Calculate phenotypic diversity (average difference in traits)
    const traitDiversity = this.calculateTraitDiversity(population);

    // Calculate average pairwise distance between agents
    let totalDistance = 0;
    let pairCount = 0;
    for (let i = 0; i < population.length; i++) {
      for (let j = i + 1; j < population.length; j++) {
        const agent1 = population[i];
        const agent2 = population[j];
        const dist = Math.abs((agent1.fitnessScore || 0) - (agent2.fitnessScore || 0));
        totalDistance += dist;
        pairCount++;
      }
    }
    const averagePairwiseDistance = pairCount > 0 ? totalDistance / pairCount : 0;

    return {
      phenotypicDiversity: traitDiversity,
      genetic_variance: Math.sqrt(geneticVariance),
      trait_distribution: traitDiversity,
      uniquePersonalities: personalities.size,
      specializations: Array.from(specializations),
      uniqueSpecializations: specializations.size,
      average_pairwise_distance: averagePairwiseDistance,
      averageFitness: avgFitness.toFixed(4),
      populationSize: population.length,
    };
  }

  /**
   * Calculate trait diversity in population
   */
  calculateTraitDiversity(population) {
    if (population.length < 2) return 0;

    const keyTraits = ['analyticalPower', 'creativityIndex', 'riskTolerance', 'learningRate'];
    let totalDiversity = 0;

    for (const trait of keyTraits) {
      const values = population.map(a => a.genes?.[trait] || 0);
      const mean = values.reduce((a, b) => a + b, 0) / values.length;
      const variance = values.reduce((sum, v) => sum + Math.pow(v - mean, 2), 0) / values.length;
      totalDiversity += Math.sqrt(variance);
    }

    return (totalDiversity / keyTraits.length).toFixed(4);
  }
