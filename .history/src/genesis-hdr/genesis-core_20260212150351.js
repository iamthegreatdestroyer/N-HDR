...
class AgentGenome {
  constructor(id, generation = 0, genes = {}) {
    ...
    this.genes = {
      ...
      communicationStyle: genes.communicationStyle || this.randomizeGeneTrait('communicationStyle'),
      riskTolerance: genes.riskTolerance || Math.random(),
      collaborativeTendency: genes.collaborativeTendency || Math.random(),
      
      // Capability genes
      domainSpecialization: genes.domainSpecialization || this.randomizeGeneTrait('domainSpecialization'),
      analyticalPower: genes.analyticalPower || Math.random(),
      creativityIndex: genes.creativityIndex || Math.random(),
      problemSolvingStrategy: genes.problemSolvingStrategy || this.randomizeGeneTrait('strategy'),
  ...
  randomizeGeneTrait(traitType) {
    const traits = {
      personality: ['analytical', 'creative', 'pragmatic', 'empathetic', 'bold'],
      communicationStyle: ['verbose', 'concise', 'technical', 'narrative', 'visual'],
      domainSpecialization: ['mathematics', 'language', 'vision', 'reasoning', 'synthesis'],
      strategy: ['greedy', 'exhaustive', 'heuristic', 'probabilistic', 'hybrid'],
    };
    return traits[traitType][Math.floor(Math.random() * traits[traitType].length)];
  }
  ...
  async evaluateAgent(agent) {
    ...
    for (const [taskName, taskDef] of this.taskRegistry) {
      try {
        ...
        logger.warn({ taskName, agentId: agent.id, error: error.message }, 'Task evaluation failed');
        results[taskName] = 0;
      }
    }
...
class MutationEngine {
  ...
  mutate(parentGenome, newId) {
    ...
    offspring.genes.riskTolerance = this.gaussianMutate(offspring.genes.riskTolerance);
    offspring.genes.analyticalPower = this.gaussianMutate(offspring.genes.analyticalPower);
    offspring.genes.creativityIndex = this.gaussianMutate(offspring.genes.creativityIndex);
    offspring.genes.collaborativeTendency = this.gaussianMutate(offspring.genes.collaborativeTendency);
    offspring.genes.learningRate = this.gaussianMutate(offspring.genes.learningRate, 0.001, 0.1);
    offspring.genes.memoryRetention = this.gaussianMutate(offspring.genes.memoryRetention);
    offspring.genes.adaptabilityScore = this.gaussianMutate(offspring.genes.adaptabilityScore);
    offspring.genes.resilience = this.gaussianMutate(offspring.genes.resilience);
    ...
    if (Math.random() < this.mutationRate) {
      offspring.genes.domainSpecialization = offspring.randomizeGeneTrait('domainSpecialization');
    }
    if (Math.random() < this.mutationRate) {
      offspring.genes.problemSolvingStrategy = offspring.randomizeGeneTrait('strategy');
    }
  ...
  crossover(parent1, parent2, newId) {
    const offspring = new AgentGenome(newId, Math.max(parent1.generation, parent2.generation) + 1);
    
    // Uniform crossover: each gene has 50% chance from either parent
    Object.keys(offspring.genes).forEach(geneKey => {
      offspring.genes[geneKey] = Math.random() < 0.5 
...
class PopulationManager {
  ...
  rouletteWheelSelection() {
    const fitnessSum = this.population.reduce((sum, agent) => sum + (agent.fitness || 0), 0);
    if (fitnessSum === 0) return this.population[Math.floor(Math.random() * this.population.length)];
  ...
  evolveGeneration(mutationEngine) {
    ...
    for (let i = 0; i < eliteCount; i++) {
      newPopulation.push(sorted[i].clone(`agent-${this.nextAgentId++}`, this.generation + 1));
    }
    ...
    while (newPopulation.length < this.populationSize) {
      ...
      if (Math.random() < 0.7) {
        // 70% crossover
        offspring = mutationEngine.crossover(parent1, parent2, `agent-${this.nextAgentId++}`);
      } else {
        // 30% mutation of elite
        offspring = mutationEngine.mutate(parent1, `agent-${this.nextAgentId++}`);
      }
  ...
  getBestAgent() {
    return [...this.population].sort((a, b) => (b.fitness || 0) - (a.fitness || 0))[0];
  }
}
...
class GenesisHDR extends EventEmitter {
  ...
  async initializeEpoch() {
    ...
    this.emit('epoch:initialized', { populationSize: this.config.populationSize });
  }
  ...
  async evaluatePopulation() {
    logger.info(`Evaluating population (generation ${this.populationManager.generation})`);
    this.state = 'evaluating';
    ...
    this.evolutionMetrics.peakFitness = Math.max(this.evolutionMetrics.peakFitness, bestFitness);
  ...
  evolveGeneration() {
    ...
    this.evolutionMetrics.agentsCreated += Math.ceil(this.config.populationSize * 0.9); // Elite not counted
  ...
  async broadcastBestAgents(topNCount = 5) {
    ...
    for (const agent of topAgents) {
      try {
        ...
        logger.info(`Broadcasting agent: ${agent.id} (fitness: ${agent.fitness.toFixed(3)})`);
        this.evolutionMetrics.broadcastCount++;
        this.emit('agent:broadcast', agentCard);
        
      } catch (error) {
        logger.warn({ agentId: agent.id, error: error.message }, 'Failed to broadcast agent');
      }
    }
  }
  ...
  async runEpoch() {
    ...
    try {
      ...
      for (let gen = 0; gen < this.config.generationsPerEpoch; gen++) {
        ...
        logger.info(`Generation ${stats.generation}: avgFitness=${stats.avgFitness.toFixed(4)}, bestFitness=${stats.bestFitness.toFixed(4)}`);
  ...
  calculateDiversity() {
    ...
    // Sample random pairs
    for (let i = 0; i < Math.min(100, this.populationManager.population.length); i++) {
      const a = this.populationManager.population[Math.floor(Math.random() * this.populationManager.population.length)];
      const b = this.populationManager.population[Math.floor(Math.random() * this.populationManager.population.length)];
  ...
  calculateGeneticDistance(agent1, agent2) {
    let distance = 0;
    const numericGenes = ['riskTolerance', 'analyticalPower', 'creativityIndex', 'learningRate', 'memoryRetention', 'adaptabilityScore', 'resilience'];
    
    for (const gene of numericGenes) {
      distance += Math.abs((agent1.genes[gene] || 0) - (agent2.genes[gene] || 0));
    }
