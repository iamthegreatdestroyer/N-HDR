...
export class TensorMock {
  ...
  _reshapeArray(values, shape) {
    ...
    for (let i = 0; i < shape[0]; i++) {
      result.push(this._reshapeArray(values.slice(i * size, (i + 1) * size), shape.slice(1)));
    }
    return result;
  }
   ...
   */
  matMul(other) {
    const otherTensor = other instanceof TensorMock ? other : new TensorMock(other);
  ...
  sum(axis = null, keepDims = false) {
    if (axis === null) {
      ...
      return new TensorMock([sum], keepDims ? this.shape.map(() => 1) : [], this.dtype);
    }
  ...
  mean(axis = null, keepDims = false) {
    if (axis === null) {
      ...
      return new TensorMock([mean], keepDims ? this.shape.map(() => 1) : [], this.dtype);
    }
  ...
  max(axis = null, keepDims = false) {
    if (axis === null) {
      ...
      return new TensorMock([max], keepDims ? this.shape.map(() => 1) : [], this.dtype);
    }
    return new TensorMock(this.values, this.shape, this.dtype);
  }
  ...
  min(axis = null, keepDims = false) {
    if (axis === null) {
      ...
      return new TensorMock([min], keepDims ? this.shape.map(() => 1) : [], this.dtype);
    }
    return new TensorMock(this.values, this.shape, this.dtype);
  }
  ...
  greater(other) {
    const otherValues = this._getTensorValues(other);
    const result = this.values.map((v, i) => v > (otherValues[i] || 0) ? 1 : 0);
 ...
 */
export function randomUniform(shape, minval = 0, maxval = 1, dtype = 'float32') {
  const size = shape.reduce((a, b) => a * b, 1);
