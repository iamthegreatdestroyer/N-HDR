...
class DocumentationVersion {
  ...
  async restoreVersion(version) {
    ...
    const versionEntry = this.history.find(entry => entry.version === version);
    if (!versionEntry) {
  ...
  async _calculateChanges(oldVersion, newContent) {
    ...
    changes.push(...sectionChanges.map(change => ({
      type: 'section',
      ...change
    })));
    ...
    changes.push(...metadataChanges.map(change => ({
      type: 'metadata',
      ...change
    })));
    ...
    changes.push(...exampleChanges.map(change => ({
      type: 'example',
      ...change
    })));
  ...
  _compareApis(oldContent, newContent) {
    ...
    for (const endpoint of allEndpoints) {
      ...
      if (!oldApi && newApi) {
        ...
        const paramChanges = this._compareApiParams(oldApi.params, newApi.params);
  ...
  async _commitVersion(version) {
    ...
    try {
      ...
      const message = 
        ...
        version.changes.map(change => `- ${change.type}: ${change.action}`).join('\n');

      await exec(`git commit -m "${message}"`);

      // Create tag
      await exec(`git tag -a v${version.version} -m "Documentation v${version.version}"`);
