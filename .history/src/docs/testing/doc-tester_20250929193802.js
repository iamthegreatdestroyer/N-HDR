...
class DocTester {
  ...
  async testDocumentation(files) {
    ...
    try {
      ...
      testResults.passed = testResults.tests.filter(t => t.status === 'passed').length;
      testResults.failed = testResults.tests.filter(t => t.status === 'failed').length;
      testResults.skipped = testResults.tests.filter(t => t.status === 'skipped').length;
  ...
  async _createTestFile(example) {
    const filename = `test-${Date.now()}-${Math.random().toString(36).slice(2)}.js`;
