...
class ApiTracker {
  ...
  async getChangelog(filters = {}) {
    ...
    if (filters.author) {
      return changelog.filter(entry => 
        entry.author === filters.author
      );
    }

    if (filters.impact) {
      return changelog.filter(entry => 
        entry.versionImpact.level === filters.impact
      );
    }
    ...
    if (filters.path) {
      return changelog.filter(entry =>
        entry.changes.some(change => 
          change.path.startsWith(filters.path)
        )
      );
    }
  ...
  _formatChangeMarkdown(change) {
    ...
    switch (change.type) {
      case 'endpoint':
        text += `- ${change.action.toUpperCase()} endpoint: \`${change.path}\`\n`;
        break;
      case 'method':
        text += `- ${change.action.toUpperCase()} method: \`${change.method} ${change.path}\`\n`;
        break;
      case 'parameter':
        text += `- ${change.action.toUpperCase()} parameter: \`${change.name}\` in ${change.in}\n`;
        break;
      case 'response':
        text += `- ${change.action.toUpperCase()} response: Status ${change.code}\n`;
        break;
      case 'schema':
        text += `- ${change.action.toUpperCase()} schema: \`${change.schema}\`\n`;
        break;
    }
  ...
  _formatChangeHtml(change) {
    ...
    switch (change.type) {
      case 'endpoint':
        text += `<p>${change.action.toUpperCase()} endpoint: <code>${change.path}</code></p>`;
        break;
      case 'method':
        text += `<p>${change.action.toUpperCase()} method: <code>${change.method} ${change.path}</code></p>`;
        break;
      case 'parameter':
        text += `<p>${change.action.toUpperCase()} parameter: <code>${change.name}</code> in ${change.in}</p>`;
        break;
      case 'response':
        text += `<p>${change.action.toUpperCase()} response: Status ${change.code}</p>`;
        break;
      case 'schema':
        text += `<p>${change.action.toUpperCase()} schema: <code>${change.schema}</code></p>`;
        break;
    }
