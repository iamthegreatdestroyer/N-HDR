...
class GenesisMcpServer {
    ...
    this.server = new McpServer(
      {
        ...
        version: "10.1.0",
        description:
          "Evolutionary agent breeding and genetic algorithm engine. Generates specialized AI agents through fitness-based selection.",
      },
      {
        capabilities: {
          tools: {},
      ...
      "Initiate evolutionary epoch with configuration",
      {
        ...
        properties: {
          ...
          mutationRate: {
            type: "number",
            description:
              "Probability of mutation for each gene (0-1, default: 0.15)",
            default: 0.15,
          },
          benchmarks: {
            type: "array",
            description:
              "Array of benchmark task definitions: [{name, weight}]",
            items: {
              type: "object",
              properties: {
                name: { type: "string" },
  ...
  setupEventHandlers() {
    ...
    this.genesis.on("agent:broadcast", (data) => {
      logger.info("Agent broadcast to catalog", {
        agentId: data.id,
      ...
      if (args.benchmarks && Array.isArray(args.benchmarks)) {
        for (const benchmark of args.benchmarks) {
          ...
          const taskFunc = async (genes, agentId) => {
            ...
            const geneBonus =
              genes.analyticalPower * 0.3 +
              genes.creativityIndex * 0.2 +
              genes.adaptabilityScore * 0.2;
  ...
  async handleGetSpecialists(args) {
    ...
    if (domain) {
      specialists = specialists.filter(
        (a) => a.genes.domainSpecialization === domain,
      );
    }
  ...
  async handleBreed(args) {
    try {
      ...
      const parent1 = this.genesis.populationManager.population.find(
        (a) => a.id === args.parentId1,
      );
      const parent2 = this.genesis.populationManager.population.find(
        (a) => a.id === args.parentId2,
      );
      ...
      const offspring = this.genesis.mutationEngine.crossover(
        parent1,
        parent2,
        newId,
      );
