...
class AgentCardValidator {
  ...
  registerCard(card) {
    ...
    // Ensure timestamps
    result.card.registered_at =
      result.card.registered_at || new Date().toISOString();
  ...
  searchCards(criteria = {}) {
    ...
    for (const card of this.validatedCards.values()) {
      ...
      if (
        ...
        card.specialization !== criteria.specialization
      ) {
        matches = false;
      }
      ...
      if (
        criteria.minFitness &&
        card.genetic_lineage?.fitness_score < criteria.minFitness
      ) {
        matches = false;
      }
  ...
  getStatistics() {
    ...
    return {
      ...
      average_fitness:
        fitnessSamples > 0 ? (totalFitness / fitnessSamples).toFixed(2) : 0,
      average_trust_score:
        cards.length > 0
          ? (
              cards.reduce((sum, c) => sum + (c.trust_score || 50), 0) /
              cards.length
            ).toFixed(2)
          : 0,
    };
  ...
  static createGenesisCard(agent, mcpEndpoint) {
    return {
      ...
      genetic_lineage: {
        ...
        fitness_score: agent.fitnessScore || 0,
