...
class TemporalDecayModel {
  ...
  computeStrength(temporal, atTime) {
    ...
    const stage =
      CONSOLIDATION_STAGES[temporal.consolidationStageIndex] ??
      CONSOLIDATION_STAGES[0];
    const emotionBoost = 1 + this._compositeEmotionalScore(temporal.emotional);
    const effectiveHalfLife =
      temporal.halfLifeMs * stage.multiplier * emotionBoost;

    const strength =
      temporal.strength *
      Math.exp((-this.lambda * elapsed) / effectiveHalfLife);
  ...
  diagnose(temporal) {
    ...
    if (
      temporal.profile !== DecayProfile.IMMORTAL &&
      currentStrength > this.minStrength
    ) {
      timeToForget =
        (effectiveHalfLife / this.lambda) *
  ...
  _attemptConsolidation(temporal) {
    ...
    const adjustedThreshold =
      nextStage.minRecalls / this.consolidationAcceleration;
