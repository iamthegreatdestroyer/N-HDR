...
import AssociativeRecall, {
  AssociationType,
} from "./associations/associative-recall.js";
...
class EchoHDRCore extends EventEmitter {
  ...
  constructor(opts = {}) {
    ...
    this._config = {
      ...
      defaultRecallLimit:
        opts.config?.defaultRecallLimit ?? DEFAULT_RECALL_LIMIT,
    };
  }
  ...
  async store(params = {}) {
    ...
    this.associativeRecall.embed(episode).catch(() => {
  ...
  async importState(state) {
    ...
    await this.associativeRecall.embedBatch(all).catch(() => {
      /* non-fatal */
    });
