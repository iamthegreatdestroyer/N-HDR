...
class EpisodicEngine extends EventEmitter {
  ...
  addAssociation(idA, idB) {
    ...
    if (!this.associationGraph.has(idA)) this.associationGraph.set(idA, new Set());
    if (!this.associationGraph.has(idB)) this.associationGraph.set(idB, new Set());
  ...
  importAll(data) {
    ...
    for (const item of data) {
      ...
      for (const tag of (item.context ?? [])) {
        const lower = tag.toLowerCase();
        if (!this.contextIndex.has(lower)) this.contextIndex.set(lower, new Set());
  ...
  _forceEvictWeakest() {
    ...
    const entries = [];
    for (const [id, episode] of this.episodes) {
      entries.push({ id, strength: this.decayModel.computeStrength(episode.temporal) });
    }
