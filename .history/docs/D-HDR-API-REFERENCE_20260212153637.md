...
console.log(`Constraint satisfaction: ${exploration.constraints.satisfied}/${exploration.constraints.satisfied + exploration.constraints.violated}`);

// Show top 5 outcomes
exploration.consequences.slice(0, 5).forEach((c, i) => {
  console.log(`  ${i+1}. ${c.outcome.primary} (likelihood: ${c.outcome.likelihood})`);
});
```
...
```javascript
const similarity = ConsequenceEmbedding.computeSimilarity(embedding1, embedding2);
