...
for (let generation = 0; generation < 50; generation++) {
  ...
  console.log(`Generation ${generation}: Best fitness = ${population[0].fitnessScore}`);
}
```
...
timeSeries.points.forEach((point, index) => {
  console.log(`Week ${index}: $${point.value.toFixed(2)} ±${point.uncertainty.toFixed(2)}`);
});
```
...
console.log(`
  ...
  By Dimension:
  ${Object.entries(impact.by_dimension).map(([dim, score]) =>
...
similar.forEach(sim => {
  console.log(`  - ${sim.decision}: similarity ${(sim.score * 100).toFixed(1)}%`);
});
```
...
console.log(`Transition path from ${scenario1.decision} to ${scenario2.decision}:`);
...
const ensemble = diffusion.exploreDecisionSpace('major_decision', {
  economicTarget: 0.8,
...
console.log(`
  ...
  Guidance Effect: ${(
    (guided.predicted_outcome.economicImpact - free.predicted_outcome.economicImpact) * 100
  ).toFixed(1)}%
`);
```
...
orchestrator.on('agent:bred', (event) => {
  console.log(`New agent bred: ${event.agent.id} (fitness: ${event.agent.fitnessScore})`);
...
orchestrator.on('consequence:explored', (event) => {
  console.log(`Consequence space explored with ${event.ensemble.length} variants`);
  
  // Publish agent card
  const card = AgentCardFactory.createGenesisCard(event.agent, 'stdio://genesis');
  validator.registerCard(card);
});
```
...
```javascript
// Diagnostic check
const avgFitness = population.reduce((sum, a) => sum + a.fitnessScore, 0) / population.length;
...
const accuracy = predictions.filter((p, i) => 
  Math.abs(p.predicted_success_rate - actualOutcomes[i]) < 0.1
