...
const mcp_registry =
  process.env.MCP_REGISTRY || "http://localhost:9000/mcp";
...
class KeyRotationManager {
  ...
  async shouldRotate(agent_id) {
    ...
    const time_since_rotation =
      new Date() - agent_keys.last_rotation;
...
class SwarmAgent {
  ...
  async execute(task) {
    if (this.state !== "ready") {
      throw new Error(`Agent ${this.agent_id} is not ready (state: ${this.state})`);
    }
    ...
    try {
      ...
      this.state = "ready";
      this.emit("task_completed", { agent_id: this.agent_id, result: response });
...
class SwarmOrchestrator {
  ...
  async distributeTask(task) {
    ...
    for (const [agent_id, agent] of this.agents) {
      if (
        agent.state === "ready" &&
        agent.messages.length < min_queue_size
      ) {
        best_agent = agent;
  ...
  async scaleToQueueDepth() {
    ...
    if (queue_depth > active_agents * 5 && active_agents < this.max_agents) {
      ...
      for (let i = 0; i < active_agents - Math.max(this.min_agents, active_agents - 2); i++) {
