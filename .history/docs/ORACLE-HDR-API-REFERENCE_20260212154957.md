...
forecast.timestamps.forEach((ts, i) => {
  ...
  console.log(
    `${dt.toISOString()}: ${forecast.forecast.expected[i].toFixed(2)} ` +
...
Object.entries(impact.stakeholders).forEach(([group, data]) => {
  if (data.affected) {
    console.log(
      `  ${group}: sentiment=${data.sentiment}, magnitude=${data.magnitude}`,
    );
  }
});
...
topAgents.forEach((agent, i) => {
  console.log(
    `${i + 1}. ${agent.id} - success prob: ${agent.lastPrediction.outcome.successProbability}`,
  );
});
```
...
oracle.on("prediction:made", (event) => {
  console.log(
    `Predicted ${event.agentId}: success=${event.prediction.outcome.successProbability}`,
  );
});
```
...
console.log(
  `Basic prediction confidence: ${prediction.outcome.confidenceLevel}`,
);
