  ...
  async syncToHDR(commander, domainId) {
    ...
    try {
      ...
      for (const record of records) {
        await commander.executeCommand(
          'omniscient-hdr',
          'crystallize',
          {
            data: record,
      ...
      await commander.executeCommand(
        'nano-swarm-hdr',
        'terminateSwarm',
        { swarmId: swarm.id, mode: 'graceful' }
      );
...
class CustomHDRApplication extends EventEmitter {
  ...
  async executeWorkflow(workflowConfig) {
    ...
    for (const step of workflowConfig.steps) {
      ...
      if (step.passToNext) {
        const nextStep = workflowConfig.steps[workflowConfig.steps.indexOf(step) + 1];
        if (nextStep) {
...
class BatchProcessor {
  ...
  async processBatch(items, config) {
    ...
    // Terminate swarm
    await this.commander.executeCommand(
      'nano-swarm-hdr',
      'terminateSwarm',
      { swarmId: swarm.id, mode: 'graceful' }
    );
...
```javascript
const swarm = await commander.executeCommand('nano-swarm-hdr', 'deploySwarm', config);
