...
const avgFitness = evaluated.reduce((sum, a) => sum + a.fitnessScore, 0) / evaluated.length;
...
genesis.on('agent:bred', (event) => {
  console.log(`Agent ${event.agent.id} bred with fitness ${event.agent.fitnessScore}`);
});
```
