...
function fixImportsInFile(filePath) {
  ...
  let newContent = content.replace(importPattern, (match, prefix, importPath, suffix) => {
  ...
  const namedToDefaultPattern = /import\s+\{\s*(SecurityManager|QuantumProcessor|ConsciousnessLayer|NanoSwarmHDR|ConceptualSwarmDeployer|ConsciousnessStateTransferProtocol|KnowledgeCrystallizer|ExpertiseEngine|CrystallineStorage)\s*\}\s+from\s+(['"].*?['"];?)/g;
  
  newContent = newContent.replace(namedToDefaultPattern, (match, className, fromClause) => {
    modified = true;
