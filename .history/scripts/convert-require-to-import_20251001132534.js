...
function convertRequireToImport(filePath) {
  ...
  const requirePattern1 = /^const\s+(\w+)\s*=\s*require\(["']([^"']+)["']\);?$/gm;
  newContent = newContent.replace(requirePattern1, (match, className, modulePath) => {
    modified = true;
    // Add .js extension if it's a relative path and doesn't have one
    if ((modulePath.startsWith('./') || modulePath.startsWith('../')) && !modulePath.endsWith('.js')) {
      modulePath += '.js';
    }
  ...
  const requirePattern2 = /^const\s+\{([^}]+)\}\s*=\s*require\(["']([^"']+)["']\);?$/gm;
  newContent = newContent.replace(requirePattern2, (match, imports, modulePath) => {
    modified = true;
    // Add .js extension if it's a relative path and doesn't have one
    if ((modulePath.startsWith('./') || modulePath.startsWith('../')) && !modulePath.endsWith('.js')) {
      modulePath += '.js';
  ...
  if (modified) {
    ...
    console.log(`✓ Converted require to import: ${path.relative(rootDir, filePath)}`);
    return true;
  }
