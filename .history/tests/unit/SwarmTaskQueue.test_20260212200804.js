...
describe("SwarmTaskQueue", () => {
  ...
  test("should respect priority ordering", () => {
    task_queue.enqueueTask("low_priority", { data: "low" }, { priority: "low" });
  ...
  test("should dequeue tasks in FIFO order within same priority", () => {
    ...
    const id1 = task_queue.enqueueTask("task1", { id: 1 }, { priority: "normal" })
      .task_id;
    const id2 = task_queue.enqueueTask("task2", { id: 2 }, { priority: "normal" })
      .task_id;
    const id3 = task_queue.enqueueTask("task3", { id: 3 }, { priority: "normal" })
      .task_id;
  ...
  test("should handle task failure and retry", () => {
    const enqueue_result = task_queue.enqueueTask("test_task", {}, { priority: "high" });
    const task_id = enqueue_result.task_id;
