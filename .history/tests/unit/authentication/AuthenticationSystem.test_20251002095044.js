...
describe('AuthenticationSystem', () => {
  ...
  beforeAll(async () => {
    ...
    AuthenticationSystem = class {
      ...
      _generateToken(user) {
        return `hdr-token-${user.id}-${Date.now()}`;
      }
    };
  });
  ...
  describe('User Authentication', () => {
    ...
    test('should reject invalid password', async () => {
      await expect(authSystem.authenticate('testuser', 'wrongpassword'))
        .rejects.toThrow('Invalid credentials');
    });

    test('should reject non-existent user', async () => {
      await expect(authSystem.authenticate('nonexistent', 'password'))
        .rejects.toThrow('Invalid credentials');
    });
  ...
  describe('Token Management', () => {
    ...
    test('should reject invalid token', async () => {
      await expect(authSystem.verifyToken('invalid-token'))
    ...
    test('should handle token expiration', async () => {
      ...
      await expect(shortLivedAuth.verifyToken(result.token))
    ...
    test('should invalidate token on logout', async () => {
      ...
      await expect(authSystem.verifyToken(token))
        .rejects.toThrow('Invalid token');
    });
  });
  ...
  describe('User Registration', () => {
    ...
    test('should accept custom roles', async () => {
      await authSystem.registerUser('admin', 'password', ['admin', 'developer']);
    ...
    test('should prevent duplicate usernames', async () => {
      ...
      await expect(authSystem.registerUser('duplicate', 'password'))
  ...
  describe('Role-Based Access Control (RBAC)', () => {
    ...
    test('should verify multiple roles', async () => {
      await authSystem.registerUser('multi', 'password', ['developer', 'analyst']);
  ...
  describe('Permission-Based Access Control', () => {
    ...
    test('should grant admin full permissions', async () => {
      ...
      expect(await authSystem.hasPermission(auth.user.id, 'manage-users')).toBe(true);
    });
    ...
    test('should grant developer limited permissions', async () => {
      ...
      expect(await authSystem.hasPermission(auth.user.id, 'delete')).toBe(false);
    });
  ...
  describe('Session Management', () => {
    test('should maintain multiple concurrent sessions', async () => {
      ...
      await expect(authSystem.verifyToken(session1.token)).resolves.toBeDefined();
      await expect(authSystem.verifyToken(session2.token)).resolves.toBeDefined();
    });
  ...
  describe('Security Tests', () => {
    ...
    test('should handle concurrent authentication attempts', async () => {
      ...
      const promises = Array(100).fill(null).map(() =>
        authSystem.authenticate('concurrent', 'password')
      );
  ...
  describe('OAuth/OIDC Integration (Mock)', () => {
    test('should support OAuth flow', async () => {
      ...
      // Register OAuth user
      await authSystem.registerUser(`oauth-${oauthUser.providerId}`, 'oauth-token', ['user']);
      const auth = await authSystem.authenticate(`oauth-${oauthUser.providerId}`, 'oauth-token');
  ...
  describe('Performance Tests', () => {
    test('should handle high authentication volume', async () => {
      ...
      await Promise.all(users.map(username =>
