...
describe('VoidBladeHDR', () => {
  ...
  beforeAll(async () => {
    const module = await import('../../../src/core/void-blade-hdr/VoidBladeHDR.js');
  ...
  describe('Constructor', () => {
    ...
    test('should support 9 security levels', () => {
      const levels = ['minimal', 'low', 'standard', 'elevated', 'high', 'critical', 'maximum', 'absolute', 'quantum-fortress'];
  ...
  describe('createSecurityZone()', () => {
    ...
    test('should enable auto-scaling', async () => {
      const zone = await voidBladeHDR.createSecurityZone(['test'], 'high', { autoScale: true });
  ...
  describe('protectResource()', () => {
    ...
    test('should apply quantum encryption at high levels', async () => {
      ...
      const protected = await voidBladeHDR.protectResource(resource, 'quantum-fortress');
  ...
  describe('Performance Tests', () => {
    ...
    test('should scale protection efficiently', async () => {
      const resources = Array(1000).fill(null).map((_, i) => ({ id: `resource-${i}` }));
      
      const start = Date.now();
      await Promise.all(resources.slice(0, 100).map(r => voidBladeHDR.protectResource(r, 'standard')));
      const duration = Date.now() - start;
...
describe('HypersonicProtection', () => {
  ...
  beforeAll(async () => {
    const module = await import('../../../src/core/void-blade-hdr/HypersonicProtection.js');
...
describe('QuantumFieldDistortion', () => {
  ...
  beforeAll(async () => {
    const module = await import('../../../src/core/void-blade-hdr/QuantumFieldDistortion.js');
...
describe('PerceptionNullifier', () => {
  ...
  beforeAll(async () => {
    const module = await import('../../../src/core/void-blade-hdr/PerceptionNullifier.js');
...
describe('SelectiveTargeting', () => {
  ...
  beforeAll(async () => {
    const module = await import('../../../src/core/void-blade-hdr/SelectiveTargeting.js');
  ...
  describe('selectTargets()', () => {
    ...
    test('should prioritize high-severity threats', async () => {
      ...
      const selected = await targeting.selectTargets(threats);
      expect(selected[0].severity).toBeGreaterThan(selected[selected.length - 1].severity);
    });
  });
});
