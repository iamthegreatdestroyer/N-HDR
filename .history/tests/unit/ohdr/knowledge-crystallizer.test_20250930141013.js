...
import KnowledgeCrystallizer from '../../src/ohdr/KnowledgeCrystallizer';
...
describe('KnowledgeCrystallizer', () => {
  ...
  beforeEach(() => {
    ...
    mockQuantumProcessor = {
      ...
      processState: jest.fn().mockResolvedValue({ /* quantum state */ }),
  ...
  describe('Initialization', () => {
    test('should initialize successfully', async () => {
      ...
      expect(mockSecurityManager.getOperationToken).toHaveBeenCalledWith('crystallize');
    ...
    test('should fail initialization with invalid security token', async () => {
      ...
      await expect(crystallizer.initialize()).rejects.toThrow('Invalid security context');
    });
  });
  ...
  describe('Error Handling', () => {
    ...
    test('should handle quantum processing errors', async () => {
      mockQuantumProcessor.processState.mockRejectedValue(new Error('Quantum error'));
      const result = await crystallizer.crystallize(mockConsciousnessState);
      expect(result.success).toBe(false);
  ...
  describe('Performance and Optimization', () => {
    ...
    test('should optimize crystal formations', async () => {
      ...
      for (let i = 1; i < sortedCrystals.length; i++) {
        expect(sortedCrystals[i-1].stability.overall)
          .toBeGreaterThanOrEqual(sortedCrystals[i].stability.overall);
      }
    });
    ...
    test('should maintain acceptable entropy levels', async () => {
      const result = await crystallizer.crystallize(mockConsciousnessState);
      for (const crystal of result.crystals) {
        expect(crystal.entropy).toBeLessThanOrEqual(config.ohdr.entropyTolerance);
      }
    });
  });
});