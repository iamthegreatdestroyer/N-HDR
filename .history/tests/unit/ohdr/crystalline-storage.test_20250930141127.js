...
describe('CrystallineStorage', () => {
  ...
  beforeEach(() => {
    ...
    mockQuantumProcessor = {
      ...
      generateState: jest.fn().mockResolvedValue({ /* quantum state */ })
    };
  ...
  describe('Initialization', () => {
    test('should initialize successfully', async () => {
      ...
      expect(mockSecurityManager.getOperationToken).toHaveBeenCalledWith('storage');
    ...
    test('should fail initialization with invalid security token', async () => {
      ...
      await expect(storage.initialize()).rejects.toThrow('Invalid security context');
    });
  });
  ...
  describe('Error Handling', () => {
    ...
    test('should handle storage errors gracefully', async () => {
      mockSecurityManager.encryptData.mockRejectedValue(new Error('Encryption failed'));
      const result = await storage.storeCrystal(mockCrystal);
    ...
    test('should handle retrieval errors gracefully', async () => {
      mockSecurityManager.decryptData.mockRejectedValue(new Error('Decryption failed'));
      await storage.storeCrystal(mockCrystal);
      const retrieved = await storage.retrieveCrystal(mockCrystal.id);
      expect(retrieved).toBeNull();
    });
