...
describe('QuantumHDR', () => {
  ...
  describe('exploreFutures()', () => {
    ...
    test('should support different exploration strategies', async () => {
      await quantumHDR.initializeQuantumSpace({});
      const breadthFirst = await quantumHDR.exploreFutures({ strategy: 'breadth-first' });
      const depthFirst = await quantumHDR.exploreFutures({ strategy: 'depth-first' });
  ...
  describe('optimizeOutcomes()', () => {
    ...
    test('should achieve 0.001 probability precision', async () => {
      const result = await quantumHDR.optimizeOutcomes([{ probability: 0.5555 }]);
  ...
  describe('Performance Tests', () => {
    test('should handle 1M pathways efficiently', async () => {
      const space = await quantumHDR.initializeQuantumSpace({ pathwayCount: 1000000 });
...
describe('ProbabilityStateManager', () => {
  ...
  beforeAll(async () => {
    const module = await import('../../../src/core/quantum-hdr/ProbabilityStateManager.js');
