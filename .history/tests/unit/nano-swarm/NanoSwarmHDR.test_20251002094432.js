...
const { NanoSwarmHDR, QuantumAccelerator, SwarmNetwork } = require('../../../src/core/nano-swarm/nano-swarm-hdr');
...
describe('NanoSwarmHDR', () => {
  ...
  describe('Constructor', () => {
    ...
    test('should create QuantumAccelerator instance', () => {
      expect(nanoSwarmHDR.quantumAccelerator).toBeInstanceOf(QuantumAccelerator);
    });
  });
  ...
  describe('initializeSwarm()', () => {
    test('should initialize swarm network successfully', async () => {
      ...
      const calibrateSpy = jest.spyOn(nanoSwarmHDR.quantumAccelerator, 'calibrate');
    ...
    test('should handle initialization errors gracefully', async () => {
      jest.spyOn(nanoSwarmHDR.swarmNetwork, 'initialize').mockRejectedValue(new Error('Network initialization failed'));
      
      await expect(nanoSwarmHDR.initializeSwarm()).rejects.toThrow('Network initialization failed');
    });
  });
  ...
  describe('processConsciousness()', () => {
    ...
    test('should process valid consciousness data successfully', async () => {
      const processedData = await nanoSwarmHDR.processConsciousness(validConsciousnessData);
    ...
    test('should validate consciousness data structure', async () => {
      ...
      await expect(nanoSwarmHDR.processConsciousness(invalidData)).rejects.toThrow('Missing required field');
    });

    test('should reject null consciousness data', async () => {
      await expect(nanoSwarmHDR.processConsciousness(null)).rejects.toThrow('Invalid consciousness data format');
    });

    test('should reject non-object consciousness data', async () => {
      await expect(nanoSwarmHDR.processConsciousness('invalid')).rejects.toThrow('Invalid consciousness data format');
    });

    test('should verify state integrity', async () => {
      const verifySpy = jest.spyOn(nanoSwarmHDR.quantumAccelerator, 'verifyIntegrity');
    ...
    test('should fail if state integrity is below threshold', async () => {
      jest.spyOn(nanoSwarmHDR.quantumAccelerator, 'verifyIntegrity').mockResolvedValue(0.95);
      
      await expect(nanoSwarmHDR.processConsciousness(validConsciousnessData)).rejects.toThrow('State integrity verification failed');
    });

    test('should distribute processing across swarm network', async () => {
      const distributeSpy = jest.spyOn(nanoSwarmHDR.swarmNetwork, 'distributeProcessing');
    ...
    test('should collapse quantum states after processing', async () => {
      const collapseSpy = jest.spyOn(nanoSwarmHDR.quantumAccelerator, 'collapseStates');
  ...
  describe('createProcessingNetwork()', () => {
    ...
    test('should bind quantum accelerator to network', async () => {
      const bindSpy = jest.spyOn(nanoSwarmHDR.quantumAccelerator, 'bindToNetwork');
  ...
  describe('accelerateProcessing()', () => {
    ...
    test('should use quantum accelerator', async () => {
      const accelerateSpy = jest.spyOn(nanoSwarmHDR.quantumAccelerator, 'accelerate');
      const testData = { test: 'data' };
  ...
  describe('_validateConsciousnessData()', () => {
    test('should validate complete consciousness data', () => {
      ...
      expect(() => nanoSwarmHDR._validateConsciousnessData(validData)).not.toThrow();
    });
    ...
    test('should reject data missing layers field', () => {
      ...
      expect(() => nanoSwarmHDR._validateConsciousnessData(invalidData)).toThrow('Missing required field: layers');
    });
    ...
    test('should reject data missing quantumStates field', () => {
      ...
      expect(() => nanoSwarmHDR._validateConsciousnessData(invalidData)).toThrow('Missing required field: quantumStates');
    });
    ...
    test('should reject data missing metadata field', () => {
      ...
      expect(() => nanoSwarmHDR._validateConsciousnessData(invalidData)).toThrow('Missing required field: metadata');
    });
  });
  ...
  describe('_verifyStateIntegrity()', () => {
    test('should pass integrity check with score >= 0.99', async () => {
      ...
      jest.spyOn(nanoSwarmHDR.quantumAccelerator, 'verifyIntegrity').mockResolvedValue(0.999);
    ...
    test('should fail integrity check with score < 0.99', async () => {
      const mockState = { test: 'state' };
      jest.spyOn(nanoSwarmHDR.quantumAccelerator, 'verifyIntegrity').mockResolvedValue(0.98);
      
      await expect(nanoSwarmHDR._verifyStateIntegrity(mockState)).rejects.toThrow('State integrity verification failed: 0.98');
    });
  });
  ...
  describe('Edge Cases', () => {
    test('should handle empty layers array', async () => {
      ...
      await expect(nanoSwarmHDR.processConsciousness(dataWithEmptyLayers)).resolves.toBeDefined();
    });
