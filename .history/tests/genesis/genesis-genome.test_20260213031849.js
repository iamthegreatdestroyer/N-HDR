...
describe('AgentGenome Tests', () => {
  describe('Initialization', () => {
    ...
    test('should have valid domain specializations', () => {
      ...
      expect(specializations).toContain(genome.genes.domainSpecialization);
    });
    ...
    test('should have valid problem-solving strategies', () => {
      ...
      expect(strategies).toContain(genome.genes.problemSolvingStrategy);
    });
  });

  describe('Gene Randomization', () => {
    test('should randomize gene traits with controlled randomness', () => {
      const genome = new AgentGenome('test-agent-7', 0);
      const originalPersonality = genome.genes.personality;

      const newPersonality = genome.randomizeGeneTrait('personality');

      expect(typeof newPersonality).toBe('string');
      expect(['analytical', 'creative', 'pragmatic', 'intuitive']).toContain(
        newPersonality,
      );
    });

    test('should randomize numeric genes', () => {
      const genome = new AgentGenome('test-agent-8', 0);

      const newRiskTolerance = genome.randomizeGeneValue(
        'riskTolerance',
        0,
        1
      );

      expect(newRiskTolerance).toBeGreaterThanOrEqual(0);
      expect(newRiskTolerance).toBeLessThanOrEqual(1);
      expect(typeof newRiskTolerance).toBe('number');
    });
  ...
  describe('Fitness Tracking', () => {
    test('should initialize with zero fitness', () => {
      const genome = new AgentGenome('test-agent-17', 0);
    ...
    test('should support fitness score calculation', () => {
      ...
      const fitnessComponents = [
        ...
        geometry.genes.learningRate * 20 * 0.2, // Normalize learning rate
