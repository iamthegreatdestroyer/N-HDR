...
describe('PopulationManager', () => {
  ...
  describe('tournamentSelection()', () => {
    ...
    it('should respect tournament size parameter', () => {
      ...
      // Tournament size=population should likely select best
      const selected10 = populationManager.tournamentSelection(10);
      expect(selected10.fitness).toBe(populationManager.getBestFitness());
    });
  ...
  describe('evolveGeneration()', () => {
    ...
    it('should preserve elite agents (top 10%)', () => {
      ...
      const newElite = [...populationManager.population]
        .sort((a, b) => (b.fitness || 0) - (a.fitness || 0))
        .slice(0, eliteCount)
        .map(a => a.fitness)
        .sort((a, b) => b - a);

      // Best agents should still be highly fit (preserving quality)
      expect(newElite[0]).toBeGreaterThanOrEqual(originalElite[0] * 0.8);
    });
    ...
    it('should handle population with varied fitness', () => {
      ...
      expect(stats.bestFitness).toBeGreaterThan(0.5);
