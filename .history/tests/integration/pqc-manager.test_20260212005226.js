...
import {
  PQCManager,
  PQCSecurityLevel,
} from "../../src/security/pqc/pqc-manager.js";
    ...
    for (const level of Object.values(PQCSecurityLevel)) {
      describe(`at ${level} level`, () => {
        ...
        it("should encapsulate and decapsulate a shared secret", () => {
          ...
          const { ciphertext, sharedSecret: encSecret } = pqc.encapsulate(
            keys.publicKey,
            level,
          );

          const { sharedSecret: decSecret } = pqc.decapsulate(
            ciphertext,
            keys.secretKey,
            level,
          );
        ...
        it("should produce different shared secrets for different keypairs", () => {
          ...
          const { sharedSecret: secretA } = pqc.encapsulate(
            keysA.publicKey,
            level,
          );
          const { sharedSecret: secretB } = pqc.encapsulate(
            keysB.publicKey,
            level,
          );
    ...
    for (const level of Object.values(PQCSecurityLevel)) {
      describe(`at ${level} level`, () => {
        ...
        it("should reject wrong public key", () => {
          ...
          const { signature } = pqc.sign("test", keysA.secretKey, level);
