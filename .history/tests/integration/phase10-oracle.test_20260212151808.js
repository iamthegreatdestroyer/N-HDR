...
} = require('../src/oracle-hdr/oracle-core');
...
describe('Phase 10.2: ORACLE-HDR - Predictive Intelligence', () => {
  ...
  describe('CausalInferenceEngine - Causal Relationships', () => {
    ...
    test('estimates intervention effect on target', () => {
      ...
      const effect = causalEngine.estimateInterventionEffect('code_change', 'revenue');
  ...
  describe('ORACLE-HDR Prediction', () => {
    ...
    test('adjusts prediction based on context', () => {
      ...
      const pred2 = oracle.predictConsequence('deploy', { historicalPrecedent: true });

      expect(pred2.baselineProbability).toBeGreaterThan(pred1.baselineProbability);
    });
