...
const { GenesisHDR } = require('../src/genesis/genesis-core');
const { OracleHDR } = require('../src/oracle/oracle-core');
const { DiffusionHDR } = require('../src/d-hdr/diffusion-core');
const { AgentCardFactory } = require('../src/agent-card/schema');
const { HDRIntegrationOrchestrator } = require('../src/hdr-orchestrator');
...
describe('Phase 10.5: Combined System Integration', () => {
  ...
  describe('Workflow: Genesis → Oracle → D-HDR', () => {
    ...
    test('generates consequence variations with D-HDR', () => {
      ...
      const consequences = diffusion.exploreDecisionSpace('deploy_agent', {
  ...
  describe('Multi-Agent Ecosystem', () => {
    test('breeds population and evaluates with ORACLE', () => {
      ...
      expect(predictions.every(p => p.predicted_success_rate !== undefined)).toBe(true);
    });
    ...
    test('selects best agent from ecosystem', () => {
      ...
      const best = predictions.reduce((best, current) =>
        current.prediction.predicted_success_rate > best.prediction.predicted_success_rate
          ? current
          : best
      );
  ...
  describe('Performance & Scalability', () => {
    ...
    test('handles concurrent D-HDR explorations', async () => {
      const explorations = Array(5)
        ...
        .map(() =>
          Promise.resolve(
            diffusion.exploreDecisionSpace(`exploration_${Math.random()}`, {}, 3)
          )
        );
  ...
  describe('Error Handling & Recovery', () => {
    test('handles invalid agent data gracefully', () => {
      const invalidAgent = { /* missing required fields */ };
  ...
  describe('Monitoring & Observability', () => {
    ...
    test('collects metrics for all components', () => {
      ...
      expect(metrics.diffusion.explorations_performed).toBeGreaterThanOrEqual(0);
    });
