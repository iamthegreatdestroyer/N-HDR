...
describe("MCP Server Architecture", () => {
  ...
  describe("schema validation patterns", () => {
    it("should validate required fields", () => {
      ...
      expect(() => schema.parse({ weights: { layer1: [1, 2, 3] } })).not.toThrow();
    ...
    it("should validate ECHO-HDR emotional metadata schema", () => {
      ...
      // Valid: partial
      expect(() =>
        emotionalSchema.parse({ significance: 0.5 })
      ).not.toThrow();
      ...
      expect(() =>
        emotionalSchema.parse({ significance: 1.5 })
      ).toThrow();

      // Invalid: negative
      expect(() =>
        emotionalSchema.parse({ urgency: -0.1 })
      ).toThrow();
    });
