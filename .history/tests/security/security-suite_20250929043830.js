  ...
  // Integrity verification tests
  {
    ...
    run: async () => {
      ...
      try {
        ...
        const verifyOriginal = await neuralHDR.verifyIntegrity(
          testData,
          originalHash
        );
        if (!verifyOriginal) {
        ...
        const verifyModified = await neuralHDR.verifyIntegrity(
          modifiedData,
          originalHash
        );
        if (verifyModified) {
  ...
  // Quantum security tests
  {
    ...
    run: async () => {
      ...
      try {
        ...
        // Verify signature
        const verifyOriginal = await neuralHDR.verifyQuantumSignature(
          testState,
          signature
        );
        if (!verifyOriginal) {
        ...
        const verifyModified = await neuralHDR.verifyQuantumSignature(
          modifiedState,
          signature
        );
        if (verifyModified) {
    ...
    run: async () => {
      ...
      try {
        ...
        const unauthResponse = await fetch(
          "http://localhost:3000/api/v1/consciousness/list"
        );
...
async function runSecurityTests() {
  ...
  console.log(
    `Security test suite complete. Passed: ${passedCount}/${results.length}`
  );
