...
describe("SecurityManager", () => {
  ...
  describe("integrity verification", () => {
    ...
    it("should detect tampering", async () => {
      ...
      const verified = await securityManager.verifyIntegrity(
        tamperedData,
        checksum
      );
  ...
  describe("error handling", () => {
    it("should handle encryption errors", async () => {
      ...
      await expect(
        securityManager.encrypt(Buffer.from("test"))
      ).rejects.toThrow();
    });
    ...
    it("should handle decryption errors", async () => {
      await securityManager.initialize();
      const invalidData = {
        ciphertext: Buffer.from("invalid"),
        iv: Buffer.alloc(16),
        tag: Buffer.alloc(16),
      };
    ...
    it("should handle integrity check errors", async () => {
      ...
      await expect(
        securityManager.verifyIntegrity(Buffer.from("test"), invalidChecksum)
      ).resolves.toBe(false);
    });
  });
  ...
  describe("utilities", () => {
    ...
    it("should validate encryption parameters", () => {
      ...
      expect(() =>
        securityManager._validateEncryptionParams(params)
      ).not.toThrow();
    });
