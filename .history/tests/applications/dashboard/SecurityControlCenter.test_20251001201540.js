  ...
  describe('Security Zones', () => {
    test('should list all security zones', async () => {
      securityCenter.getSecurityZones.mockResolvedValue({
        zones: [
          { id: 'zone-001', name: 'N-HDR-Protection', level: 'maximum', active: true },
          { id: 'zone-002', name: 'NS-HDR-Protection', level: 'maximum', active: true },
          { id: 'zone-003', name: 'O-HDR-Protection', level: 'high', active: true }
        ],
        totalZones: 3
      });
  ...
  describe('Threat Scanning', () => {
    ...
    test('should detect and report threats', async () => {
      securityCenter.scanThreats.mockResolvedValue({
        ...
        threatsFound: 1,
        threats: [
          { type: 'malware', severity: 'high', mitigated: true }
        ]
      });
  ...
  describe('Security Alerts', () => {
    test('should get security alerts', async () => {
      securityCenter.getSecurityAlerts.mockResolvedValue({
        alerts: [
          { type: 'warning', message: 'Multiple failed login attempts', timestamp: Date.now() }
        ],
        totalAlerts: 1
      });
  ...
  describe('Access Control', () => {
    test('should get access log', async () => {
      securityCenter.getAccessLog.mockResolvedValue({
        events: [
          { user: 'admin', action: 'login', timestamp: Date.now(), success: true },
          { user: 'user1', action: 'access-resource', timestamp: Date.now(), success: true }
        ],
        totalEvents: 2,
        period: '24h'
      });
