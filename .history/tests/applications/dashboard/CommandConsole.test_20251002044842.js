...
describe("Command Console", () => {
  ...
  describe("Command Execution", () => {
    ...
    test("should execute NS-HDR commands", async () => {
      ...
      const result = await console.executeCommand("nshdr.deploySwarm", {
        size: 150,
      });
      expect(result.executed).toBe(true);
    });
  ...
  describe("Command Validation", () => {
    test("should validate command syntax", async () => {
      ...
      const result = await console.validateCommand(
        "nhdr.captureState --source ai-model"
      );
