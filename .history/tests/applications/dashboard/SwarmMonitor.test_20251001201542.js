...
describe('Swarm Monitor', () => {
  ...
  describe('Swarm Status', () => {
    test('should get all active swarms', async () => {
      swarmMonitor.getActiveSwarms.mockResolvedValue({
        swarms: [
          { id: 'swarm-001', type: 'knowledge-search', size: 150, efficiency: 0.94 },
          { id: 'swarm-002', type: 'state-processing', size: 200, efficiency: 0.92 },
          { id: 'swarm-003', type: 'optimization', size: 180, efficiency: 0.96 }
        ],
        totalSwarms: 3,
        totalBots: 530
      });
    ...
    test('should get swarm details', async () => {
      ...
      const result = await swarmMonitor.getSwarmDetails({ swarmId: 'swarm-001' });
  ...
  describe('Swarm Performance', () => {
    test('should get swarm performance metrics', async () => {
      ...
      const result = await swarmMonitor.getSwarmPerformance({ swarmId: 'swarm-001' });
    ...
    test('should track performance over time', async () => {
      ...
      const result = await swarmMonitor.getSwarmMetrics({ swarmId: 'swarm-001' });
  ...
  describe('Swarm Deployment', () => {
    ...
    test('should handle deployment failures', async () => {
      swarmMonitor.deploySwarm.mockRejectedValue(new Error('Insufficient resources'));

      await expect(swarmMonitor.deploySwarm({ size: 10000 }))
  ...
  describe('Swarm Termination', () => {
    test('should terminate swarm', async () => {
      ...
      const result = await swarmMonitor.terminateSwarm({ swarmId: 'swarm-001' });
