...
describe('KnowledgeExtractionEngine', () => {
  ...
  beforeEach(() => {
    ...
    QuantumProcessor.prototype.processQuantumStates = jest.fn().mockResolvedValue({
  ...
  describe('extractKnowledge', () => {
    ...
    it('should fail for invalid results format', async () => {
      ...
      await expect(
        engine.extractKnowledge(invalidResults)
      ).rejects.toThrow('Failed to extract knowledge');
    });
  });
  ...
  describe('analyzePatterns', () => {
    ...
    it('should fail for invalid extraction ID', async () => {
      await expect(
        engine.analyzePatterns('invalid-id')
  ...
  describe('Graph Operations', () => {
    ...
    it('should prune weak nodes and edges', async () => {
      ...
      knowledge.nodes.forEach(node => {
        expect(node.weight).toBeGreaterThanOrEqual(config.knowledge.nodeThreshold);
        expect(node.connections.length).toBeGreaterThanOrEqual(config.knowledge.minConnections);
      });

      knowledge.edges.forEach(edge => {
        expect(edge.weight).toBeGreaterThanOrEqual(config.knowledge.edgeThreshold);
      });
    });
  });
});