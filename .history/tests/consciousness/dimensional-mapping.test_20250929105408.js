...
describe('DimensionalMapping', () => {
  ...
  describe('Higher Dimensional Mapping', () => {
    ...
    test('should reject invalid target dimensions', async () => {
      await expect(mapping.mapToHigherDimension(testState, 9))
        .rejects.toThrow('Target dimension exceeds maximum');
    });

    test('should maintain vector magnitudes', async () => {
      const original = new mapping.constructor.DimensionalVector(testState.coordinates);
      const mapped = await mapping.mapToHigherDimension(testState, 5);
      const mappedVector = new mapping.constructor.DimensionalVector(mapped.coordinates);
  ...
  describe('Lower Dimensional Mapping', () => {
    ...
    test('should preserve relative magnitudes', async () => {
      const original = new mapping.constructor.DimensionalVector(testState.coordinates);
      const reduced = await mapping.mapToLowerDimension(testState, 3);
      const reducedVector = new mapping.constructor.DimensionalVector(reduced.coordinates);
    ...
    test('should reject invalid target dimensions', async () => {
      await expect(mapping.mapToLowerDimension(testState, 1))
        .rejects.toThrow('Target dimension below minimum');
    });
  ...
  describe('State Compatibility', () => {
    test('should calculate compatibility between states', async () => {
      ...
      const compatibility = await mapping.calculateCompatibility(state1, state2);
