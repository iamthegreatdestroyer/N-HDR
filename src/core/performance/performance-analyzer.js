...
class PerformanceAnalyzer {
  ...
  async _performAnalysis() {
    try {
      ...
      const maxSeverity = Math.max(
        ...detectedBottlenecks.map((b) => b.severity)
      );
      this.metrics.recordMetric(
        "performance.bottlenecks.severity",
        maxSeverity
      );
    } catch (error) {
  ...
  _detectBottlenecks(metrics) {
    ...
    const cpuUtilization =
      (metrics.cpu.utilization.user /
        (metrics.cpu.utilization.user + metrics.cpu.utilization.system)) *
      100;
  ...
  _generateRecommendations() {
    ...
    if (bottleneckArray.some((b) => b.type === "CPU_SATURATION")) {
      recommendations.push({
        ...
        priority: "high",
        description:
          "Consider scaling compute resources or optimizing CPU-intensive operations",
        actions: [
          "Scale up CPU cores",
