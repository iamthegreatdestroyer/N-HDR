...
class BenchmarkingSuite {
  ...
  exportToMarkdown() {
    ...
    if (this.results.has("task-processing")) {
      ...
      markdown += `| Standard Deviation | ${taskResults.stdDev.toFixed(
        2
      )}ms |\n`;
      markdown += `| Min Duration | ${taskResults.min.toFixed(2)}ms |\n`;
      markdown += `| Max Duration | ${taskResults.max.toFixed(2)}ms |\n`;
      markdown += `| Average Memory | ${(
        taskResults.memoryStats.average /
        1024 /
        1024
      ).toFixed(2)}MB |\n`;
      markdown += `| Peak Memory | ${(
        taskResults.memoryStats.peak /
        1024 /
        1024
      ).toFixed(2)}MB |\n\n`;
    }
    ...
    if (this.swarmMetrics.size > 0) {
      ...
      markdown +=
        "| Nodes | Throughput (tasks/s) | Latency (ms/task) | Resource Utilization | Network Efficiency |\n";
      markdown +=
        "|-------|-------------------|----------------|-------------------|------------------|\n";

      for (const [nodes, metrics] of this.swarmMetrics) {
        markdown += `| ${nodes} | ${metrics.throughput.toFixed(
          2
        )} | ${metrics.latency.toFixed(2)} | ${(
          metrics.resourceUtilization * 100
        ).toFixed(1)}% | ${(metrics.networkEfficiency * 100).toFixed(1)}% |\n`;
      }
    }
