...
class ComplianceChecker {
  ...
  initializeDefaultPolicies() {
        ...
        for (const container of pod.spec.containers) {
          ...
          if (!sc) {
            ...
            if (sc.readOnlyRootFilesystem === undefined) {
              issues.push(
                `Container ${container.name} should have read-only root filesystem`,
              );
            }
          }
        }
      ...
      check: (pod) => {
        ...
        for (const container of pod.spec.containers) {
          ...
          if (image.endsWith(":latest")) {
            issues.push(
              `Container ${container.name} uses 'latest' tag (${image})`,
            );
          }

          // Check registry
          const registryAllowed = allowedRegistries.some((r) =>
            image.startsWith(r),
          );
          if (!registryAllowed && !image.includes(":")) {
            issues.push(
              `Container ${container.name} registry not in allowed list (${image})`,
            );
          }
        }
      ...
      check: (resource) => {
        ...
        if (!resource.spec?.replicas || resource.spec.replicas < 2) {
          return {
            passed: false,
            issues: [
              `Deployment has ${resource.spec?.replicas || 0} replicas, should have at least 2`,
            ],
          };
        }
    ...
    this.addPolicy("resource-ratio", {
      ...
      check: (pod) => {
        ...
        const issues = [];
        for (const container of pod.spec.containers) {
          const request = this.parseMemory(
          ...
          if (request && limit && limit < request * 2) {
            issues.push(
              `Container ${container.name} memory limit should be 2x request (${request}Mi req, ${limit}Mi limit)`,
            );
          }
        }
  ...
  async start() {
    ...
    if (this.eventBus) {
      this.eventBus.publish("complianceChecker:started", {
        timestamp: Date.now(),
      });
    }
  }
  ...
  async stop() {
    ...
    if (this.eventBus) {
      this.eventBus.publish("complianceChecker:stopped", {
        timestamp: Date.now(),
      });
    }
  }
  ...
  async performComplianceCheck(resources = []) {
    ...
    if (checkResult.violationsBySeverity.high > 0 && this.eventBus) {
      this.eventBus.publish(
        "complianceChecker:criticalViolations",
        checkResult,
      );
    }
  ...
  getPolicyDetails(policyName) {
    ...
    return {
      found: true,
      ...policy,
      affectedResources: Array.from(this.violations.values()).filter((v) =>
        v.violations.some((vi) => vi.policy === policyName),
      ).length,
    };
  }
  ...
  getViolationStatistics() {
    ...
    for (const [, data] of this.violations) {
      for (const violation of data.violations) {
        stats.byPolicy[violation.policy] =
          (stats.byPolicy[violation.policy] || 0) + 1;
  ...
  getStatistics() {
    return {
      ...
      policies: {
        ...
        enabled: Array.from(this.policies.values()).filter((p) => p.enabled)
          .length,
      },
