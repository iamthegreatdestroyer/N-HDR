...
class OrchestrationEngine extends EventEmitter {
  ...
  async start() {
    ...
    try {
      ...
      if (this.eventBus) {
        this.eventBus.publish("orchestrationEngine:started", {
          timestamp: Date.now(),
        });
      }
  ...
  async stop() {
    ...
    if (this.eventBus) {
      this.eventBus.publish("orchestrationEngine:stopped", {
        timestamp: Date.now(),
      });
    }
  ...
  async performOrchestration() {
    ...
    try {
      ...
      this.metrics.averageCycleDuration =
        this.metrics.totalCycleDuration / this.metrics.orchestrationCycles;
  ...
  analyzeSystemHealth() {
    ...
    if (avgError > 5) {
      ...
      analysis.recommendations.push(
        "Investigate error sources, trigger self-healing",
      );
    }
    ...
    for (const decision of decisions) {
      try {
        ...
        if (
          ...
          !this.budgetEnforcer?.canPerformAction(decision.estimatedCost)
        ) {
          console.warn(
            `Decision rejected due to budget constraints: ${decision.type}`,
          );
          continue;
        }
        ...
        if (this.eventBus) {
          this.eventBus.publish(
            "orchestrationEngine:decisionExecuted",
            decision,
          );
        }
      } catch (error) {
