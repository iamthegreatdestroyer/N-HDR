...
class MetricsExporter {
  ...
  async start() {
    ...
    // Subscribe to events
    if (this.eventBus) {
      this.eventBus.subscribe("request:completed", (data) =>
      ...
      this.eventBus.subscribe("cost:incurred", (data) =>
        this.onCostIncurred(data),
      );
      this.eventBus.subscribe("compliance:violation", (data) =>
        this.onViolationDetected(data),
      );
      this.eventBus.subscribe("anomaly:detected", (data) =>
        this.onAnomalyDetected(data),
      );
      this.eventBus.subscribe("healing:completed", (data) =>
        this.onHealingCompleted(data),
      );
    }
    ...
    if (this.eventBus) {
      this.eventBus.publish("metricsExporter:started", {
        timestamp: Date.now(),
      });
    }
  }
  ...
  async stop() {
    ...
    if (this.eventBus) {
      this.eventBus.publish("metricsExporter:stopped", {
        timestamp: Date.now(),
      });
    }
  }
  ...
  updateGaugeMetrics() {
    ...
    if (this.modules.circuitBreaker) {
      ...
      if (status) {
        this.metrics.circuitBreakerOpenCount = Object.values(status).filter(
          (s) => s.state === "open",
        ).length;
      }
    }
  ...
  exportMetricsPrometheus() {
    ...
    for (const [bucket, count] of Object.entries(
      this.metrics.requestLatencyBuckets,
    )) {
  ...
  resetMetrics() {
    ...
    if (this.eventBus) {
      this.eventBus.publish("metricsExporter:metricsReset", {
        timestamp: Date.now(),
      });
    }
  }
