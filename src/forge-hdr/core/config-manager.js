...
class ConfigurationManager {
  ...
  notifySubscribers(section, config) {
    ...
    subscribers.forEach((callback) => {
      try {
        ...
        console.error(
          `Error notifying subscriber for section ${section}:`,
          err,
        );
      }
    });
  ...
  async start() {
    ...
    if (this.eventBus) {
      this.eventBus.publish("configurationManager:started", {
        timestamp: Date.now(),
      });
    }
  }
  ...
  async stop() {
    ...
    if (this.eventBus) {
      this.eventBus.publish("configurationManager:stopped", {
        timestamp: Date.now(),
      });
    }
  }
  ...
  validateBudget(config) {
    ...
    if (
      ...
      (config.alertThresholdPercent < 0 || config.alertThresholdPercent > 100)
    ) {
