...
class PerformanceProfiler {
  ...
  async start() {
    ...
    if (this.eventBus) {
      this.eventBus.publish("performanceProfiler:started", {
        timestamp: Date.now(),
      });
    }
  }
  ...
  async stop() {
    ...
    if (this.eventBus) {
      this.eventBus.publish("performanceProfiler:stopped", {
        timestamp: Date.now(),
      });
    }
  }
  ...
  recordRequest(requestId, data = {}) {
    ...
    if (this.requestMetrics.size > 10000) {
      const oldest = Array.from(this.requestMetrics.entries()).sort(
  ...
  updateAggregateMetrics(metric) {
    ...
    this.metrics.averageMemoryUsage =
      (this.metrics.averageMemoryUsage + (metric.memory || 0)) / 2;
    this.metrics.averageCpuUsage =
      (this.metrics.averageCpuUsage + (metric.cpu || 0)) / 2;
  }
  ...
  collectProfileData() {
    ...
    // Keep bounded
    if (this.profilingData.length > 360) {
      // 1 hour of 10-second intervals
  ...
  updatePercentiles() {
    ...
    this.metrics.p50ResponseTime =
      allDurations[Math.floor(allDurations.length * 0.5)];
   ...
   */
  analyzePerformanceTrends(timeWindowMs = 300000) {
    // 5 minutes default
    const now = Date.now();
    const cutoff = now - timeWindowMs;

    const recentMetrics = Array.from(this.requestMetrics.values()).filter(
    ...
    const oldMetrics = Array.from(this.requestMetrics.values()).filter(
      (m) => m.timestamp <= cutoff && m.timestamp > cutoff - timeWindowMs,
    );
