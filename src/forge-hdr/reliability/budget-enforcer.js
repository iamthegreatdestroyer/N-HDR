...
class BudgetEnforcer extends EventEmitter {
  ...
  async performBudgetCheck() {
    try {
      ...
      const percentageUsed =
        (currentSpending / this.config.monthlyBudget) * 100;
  ...
  calculateBudgetStatus(current, projected) {
    const hardLimitThreshold =
      this.config.monthlyBudget * this.config.hardLimit;
    const softLimitThreshold =
      this.config.monthlyBudget * this.config.softLimit;

    if (current >= this.config.monthlyBudget) {
      return "critical";
    } else if (
      current >= hardLimitThreshold ||
      projected >= this.config.monthlyBudget
    ) {
      return "warning";
  ...
  async scaleDownNonCritical() {
    ...
    const nonCritical = (topology.workloads || []).filter(
  ...
  async applyEmergencyMeasures() {
    ...
    const totalSavings = emergency.costSavingsMeasures.reduce(
   ...
   */
  getStatistics() {
    const avgSavingsPerAction =
      this.metrics.actionsEnforced > 0
        ? (this.metrics.costsSaved / this.metrics.actionsEnforced).toFixed(2)
        : 0;
  ...
  canPerformAction(estimatedCost) {
    ...
    const afterActionPercentage =
      (afterAction / this.config.monthlyBudget) * 100;
