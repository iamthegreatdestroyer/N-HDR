...
class QuantumComponent {
  ...
  _updateQubitStates() {
    this._data.qubits.forEach((qubit, id) => {
      if (this.dashboard.controller?.quantumManager) {
        const state =
          this.dashboard.controller.quantumManager.getQubitState(id);
        if (state) {
  ...
  _updateEntanglements() {
    ...
    this._data.entanglements.forEach((pair) => {
      ...
      const status = this.dashboard.controller.quantumManager.checkEntanglement(
        id1,
        id2
      );
  ...
  _updateMetrics() {
    ...
    this._data.metrics.activeQubits = Array.from(
      this._data.qubits.values()
    ).filter((qubit) => qubit.state !== "measured").length;

    // Calculate average fidelity
    const fidelities = Array.from(this._data.qubits.values()).map(
      (qubit) => qubit.fidelity
    );
    this._data.metrics.fidelity =
  ...
  _prepareVisualizationData() {
    return {
      ...
      qubits: Array.from(this._data.qubits.entries()).map(([id, qubit]) => ({
        ...
        state: qubit.state,
        color:
          this.options.stateColors[qubit.state] ||
          this.options.stateColors.error,
  ...
  _updateDecoherenceStatus() {
    ...
    const status =
      this.dashboard.controller.quantumManager.getDecoherenceStatus();
