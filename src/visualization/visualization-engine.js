...
class VisualizationEngine {
  ...
  async _createBaseElements() {
    ...
    for (let i = 0; i < config.consciousness.layers.length; i++) {
      ...
      // Create label
      const label = this._createTextLabel(layer.name, this._getLayerColor(layer.name));
  ...
  _getLayerColor(layerName) {
    ...
    } else if (layerType.includes('timeline') || layerType.includes('conversation')) {
      return this.colorSchemes.timeline;
    } else if (layerType.includes('context') || layerType.includes('relationship')) {
      return this.colorSchemes.context;
    } else if (layerType.includes('reasoning') || layerType.includes('pathways')) {
      return this.colorSchemes.reasoning;
    } else if (layerType.includes('emotion') || layerType.includes('resonance')) {
      return this.colorSchemes.emotional;
    } else if (layerType.includes('quantum') || layerType.includes('entangled')) {
  ...
  _onMouseDown(event) {
    ...
    this.mouse.x = ((event.clientX - rect.left) / this.dimensions.width) * 2 - 1;
    this.mouse.y = -((event.clientY - rect.top) / this.dimensions.height) * 2 + 1;
  ...
  _performRaycasting() {
    ...
    const layerObjects = Array.from(this.layers.values()).map(layer => layer.sphere);
    const dataObjects = this.dataPoints.map(point => point.mesh).filter(Boolean);
    const intersects = this.raycaster.intersectObjects([...layerObjects, ...dataObjects]);
    ...
    if (intersects.length > 0) {
      ...
      const layerEntry = Array.from(this.layers.entries())
        .find(([_, layer]) => layer.sphere === object);
