...
class DreamConsolidator extends EventEmitter {
  ...
  constructor(opts = {}) {
    ...
    if (!opts.episodicEngine)
      throw new Error("DreamConsolidator requires episodicEngine");
    if (!opts.decayModel)
      throw new Error("DreamConsolidator requires decayModel");
    if (!opts.associativeRecall)
      throw new Error("DreamConsolidator requires associativeRecall");
  ...
  async consolidate(mode = ConsolidationMode.STANDARD) {
    if (this.phase !== DreamPhase.IDLE) {
      throw new Error(
        `Consolidation already in progress (phase: ${this.phase})`,
      );
    }
  ...
  async _strengthenMemories(ranked, mode) {
    const ratios = {
      [ConsolidationMode.LIGHT]: 0.1,
      [ConsolidationMode.STANDARD]: 0.2,
      [ConsolidationMode.DEEP]: 0.35,
      [ConsolidationMode.REM]: 0.15,
    };

    const topN = Math.ceil(ranked.length * (ratios[mode] ?? 0.2));
    let count = 0;
    ...
    for (let i = 0; i < topN && i < ranked.length; i++) {
      ...
      this.emit("memoryStrengthened", {
        id: episode.id,
        newStrength: this.decayModel.computeStrength(
          episode.temporal,
          Date.now(),
        ),
      });
  ...
  async _discoverInsights(ranked, mode) {
    ...
    if (
      ...
      try {
        const processed = await this.dreamHDR.processPatterns(
          this._activeDreamId,
        );
