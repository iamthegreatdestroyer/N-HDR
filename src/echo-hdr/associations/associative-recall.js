...
class AssociativeRecall {
  ...
  async search(query, opts = {}) {
    ...
    if (opts.emotionalFilter) {
      results = results.filter((r) => {
        ...
        if (
          opts.emotionalFilter.minSignificance &&
          sig < opts.emotionalFilter.minSignificance
        )
          return false;
        if (
          opts.emotionalFilter.minResonance &&
          res < opts.emotionalFilter.minResonance
        )
          return false;
        return true;
      });
  ...
  async searchByEpisode(episodeId, topK) {
    ...
    return this._localSearch(embedding, k + 1).filter(
      (r) => r.episodeId !== episodeId,
    ); // exclude self
  }
  ...
  async chainSearch(query, depth = 3, branchFactor = 3) {
    ...
    for (let d = 1; d <= Math.min(depth, MAX_CHAIN_DEPTH); d++) {
      ...
      if (nextEmbeddings.length === 0 || allResults.length >= MAX_CHAIN_RESULTS)
        break;
  ...
  hybridScore(vectorScore, emotional) {
    ...
    return 0.6 * vectorScore + 0.25 * sig + 0.1 * res + 0.05 * urg;
  }
