...
class CacheManager extends EventEmitter {
    ...
    if (this.options.l2Enabled) {
      ...
      try {
        if (this.options.strategy === "write-through") {
          ...
          this.l2Cache
            .setex(cacheKey, effectiveTTL, serialized)
            .catch((err) => {
  ...
  getMetrics() {
    const l1HitRate =
      ...
      0;
    const l2HitRate =
      this.metrics.l2.hits / (this.metrics.l2.hits + this.metrics.l2.misses) ||
      0;
    const overallHitRate =
      this.metrics.overall.hits /
        (this.metrics.overall.hits + this.metrics.overall.misses) || 0;
  ...
  _serialize(value) {
    ...
    if (
      ...
      json.length > this.options.compressionThreshold
    ) {
