...
class PerformanceAnalyzer extends EventEmitter {
  ...
  getAnalysis() {
    ...
    const analysis = {
      ...
      operations: {
        ...
        bottlenecks: Array.from(this.bottlenecks).filter(
  ...
  _collectMetrics() {
    ...
    const latencies = Array.from(this.operations.values()).map(
      (op) => Number(process.hrtime.bigint() - op.startTime) / 1e6
    );
    this.metrics.latency[this.currentIndex] =
      latencies.length > 0
  ...
  _calculateMetricStats(metric) {
    ...
    const trend = recent.length >= 2 ? this._calculateTrend(recent) : "stable";
  ...
  _generateRecommendations() {
    ...
    if (analysis.memory.trend === "increasing" && analysis.memory.avg > 75) {
      recommendations.push({
        ...
        action:
          "Check for memory leaks and optimize memory-intensive operations",
      });
    }
