...
class Optimizer extends EventEmitter {
  ...
  _initializeStrategies() {
    ...
    this.registerStrategy("latency", async (params) => {
      ...
      if (params.optimizePaths) {
        const bottlenecks = this.analyzer.getAnalysis().operations.bottlenecks;
  ...
  async _optimize(component = null) {
    if (
      ...
      this.options.maxOptimizationAttempts
    ) {
  ...
  _calculateOptimizationImpact(before, after) {
    const impact = {
      ...
      memory:
        (before.memory.current - after.memory.current) / before.memory.current,
      latency:
        (before.latency.current - after.latency.current) /
        before.latency.current,
      throughput:
        (after.throughput.current - before.throughput.current) /
