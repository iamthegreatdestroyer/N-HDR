...
class KnowledgeNavigationEngine extends EventEmitter {
  ...
  _matchesDirection(crystal, direction) {
    ...
    if (
      direction.minComplexity &&
      crystal.complexity < direction.minComplexity
    ) {
      return false;
    }

    if (
      direction.maxComplexity &&
      crystal.complexity > direction.maxComplexity
    ) {
      return false;
    }
  ...
  _scoreDirectionAlignment(crystal, direction) {
    ...
    if (direction.targetComplexity) {
      const complexityDiff = Math.abs(
        crystal.complexity - direction.targetComplexity
      );
