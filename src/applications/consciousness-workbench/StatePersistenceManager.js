...
class StatePersistenceManager extends EventEmitter {
  ...
  async initialize() {
    try {
      ...
      throw new Error(
        `Persistence manager initialization failed: ${error.message}`
      );
    }
  }
  ...
  async save(state) {
    ...
    try {
      ...
      if (
        ...
        this.storageSize > this.config.maxStorageSize
      ) {
        await this._cleanup();
      }
  ...
  async _cleanup() {
    ...
    const entries = Array.from(this.stateIndex.values()).sort(
